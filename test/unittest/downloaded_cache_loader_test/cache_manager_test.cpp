...
HWTEST_F(CacheManagerTest, GetMediaCache_NotFound_001, TestSize.Level0)
{
    auto manager = std::make_shared<DownloadedCacheManager>("/data/cache");
    ASSERT_NE(manager, nullptr);
    std::string result = manager->GetMediaCache("http://nonexistent.com/test.mp4");
    EXPECT_EQ(result, "");
}
...
HWTEST_F(CacheManagerTest, GetRootUrl_001, TestSize.Level0)
{
    auto manager = std::make_shared<DownloadedCacheManager>("/data/cache");
    ASSERT_NE(manager, nullptr);
...
HWTEST_F(CacheManagerTest, CacheMetaData_Struct_001, TestSize.Level0)
{
    CacheMetaData metadata;
...
HWTEST_F(CacheManagerTest, MultipleInstances_001, TestSize.Level0)
{
...
HWTEST_F(CacheManagerTest, EmptyCacheDir_001, TestSize.Level0)
{
    auto manager = std::make_shared<DownloadedCacheManager>("");
    ASSERT_NE(manager, nullptr);
    EXPECT_EQ(manager->GetCacheDir(), "");
}
