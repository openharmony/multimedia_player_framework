 ...
 * @tc.desc  : Test Prepare pipeline_->Prepare() != Status::OK
 */
HWTEST_F(HitranscodeImplUnitTest, Prepare_001, TestSize.Level1)
{
    transcoder_->demuxerFilter_ = demuxerFilter_;
    transcoder_->pipeline_->filters_.push_back(demuxerFilter_);
    transcoder_->demuxerFilter_->curState_ = Pipeline::FilterState::ERROR;
    transcoder_->isExistVideoTrack_ = false;
 ...
 * @tc.desc  : Test Prepare SetSurfacePipeline(width, height) == Status::ERROR_UNKNOWN
 */
HWTEST_F(HitranscodeImplUnitTest, Prepare_002, TestSize.Level1)
{
    transcoder_->demuxerFilter_ = demuxerFilter_;
    transcoder_->pipeline_->filters_.clear();
    transcoder_->isExistVideoTrack_ = false;
    int32_t ret = transcoder_->Prepare();
 ...
 * @tc.name  : Test Pause
 * @tc.number: Pause_001
 * @tc.desc  : Test Pause TransTranscoderStatus(pipeline_->Pause()) != MSERR_OK
 */
HWTEST_F(HitranscodeImplUnitTest, Pause_001, TestSize.Level1)
{
 ...
 * @tc.name  : Test Pause
 * @tc.number: Pause_002
 * @tc.desc  : Test Pause startTime_ == -1
 */
