...
namespace Media {
namespace {
constexpr HiviewDFX::HiLogLabel LABEL = {LOG_CORE, LOG_DOMAIN_PLAYER, "ScopedHapToken"};
...
ScopedHapToken::ScopedHapToken(const std::vector<std::string> &permissions, const std::string &bundleName)
{
    ...
    if (managerTokenId_ == 0 || SetSelfTokenID(managerTokenId_) != 0) {
        MEDIA_LOGE("ScopedHapToken: get or set foundation token failed");
        return;
    }
    ...
    HapPolicyParams policy = {
        ...
        .domain = "test.domain.mediaplayertdd",
        .permList = {},
    ...
    if (!valid_) {
        MEDIA_LOGE("ScopedHapToken: alloc or set HAP token failed");
    }
}
