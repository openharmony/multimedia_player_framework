...
#include "soundpoolsetvolume_fuzzer.h"
...
namespace OHOS {
namespace Media {
SoundPoolSetVolumeFuzzer::SoundPoolSetVolumeFuzzer()
{
}
...
bool SoundPoolSetVolumeFuzzer::FuzzSoundPoolSetVolume(uint8_t *data, size_t size)
{
    int maxStreams = 3;
    ...
    const string path = "/data/test/fuzztest_01.mp3";
    int32_t fd = 0;
    int32_t soundID = 0;
    ...
    float leftVolume = *reinterpret_cast<float *>(data);
    float rightVolume = *reinterpret_cast<float *>(data);
    if (soundID > 0) {
        int32_t streamID = TestSoundPool::Play(soundID, playParameters);
        sleep(waitTime1);
        if (streamID > 0) {
            if (size >= sizeof(float)) {
                TestSoundPool::SetVolume(streamID, leftVolume, rightVolume);
                sleep(waitTime1);
            }
        }
...
bool FuzzTestSoundPoolSetVolume(uint8_t *data, size_t size)
{
    auto soundPool_ = std::make_unique<SoundPoolSetVolumeFuzzer>();
    return soundPool_->FuzzSoundPoolSetVolume(data, size);
}
} // namespace OHOS
...
extern "C" int LLVMFuzzerTestOneInput(uint8_t *data, size_t size)
{
    ...
    FuzzTestSoundPoolSetVolume(data, size);
    return 0;
}