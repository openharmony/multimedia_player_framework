...
#include "soundpoolsetpriority_fuzzer.h"
...
namespace OHOS {
namespace Media {
SoundPoolSetPriorityFuzzer::SoundPoolSetPriorityFuzzer()
{
}
...
bool SoundPoolSetPriorityFuzzer::FuzzSoundPoolSetPriority(uint8_t *data, size_t size)
{
    int maxStreams = 3;
    ...
    if (ret != 0) {
        cout << "FuzzSoundPoolSetPriority set callback failed" << endl;
    }

    const string path = "/data/test/fuzztest_02.ogg";
    int32_t fd = 0;
    int32_t soundID = 0;
    ...
    int32_t priority = *reinterpret_cast<int32_t *>(data);
    if (soundID > 0) {
        int32_t streamID = TestSoundPool::Play(soundID, playParameters);
        sleep(waitTime1);
        if (streamID > 0) {
            TestSoundPool::SetPriority(streamID, priority);
            sleep(waitTime1);
        }
...
extern "C" int LLVMFuzzerTestOneInput(uint8_t *data, size_t size)
{
    ...
    FuzzTestSoundPoolSetPriority(data, size);
    return 0;
}