...
#include "soundpoolsetloop_fuzzer.h"
...
namespace OHOS {
namespace Media {
SoundPoolSetLoopFuzzer::SoundPoolSetLoopFuzzer()
{
}
...
bool SoundPoolSetLoopFuzzer::FuzzSoundPoolSetLoop(uint8_t *data, size_t size)
{
    int maxStreams = 3;
    ...
    int32_t loop = *reinterpret_cast<int32_t *>(data);
    if (soundID > 0) {
        int32_t streamID = TestSoundPool::Play(soundID, playParameters);
        sleep(waitTime1);
        if (streamID > 0) {
            TestSoundPool::SetLoop(streamID, loop);
            sleep(waitTime1);
        }
...
extern "C" int LLVMFuzzerTestOneInput(uint8_t *data, size_t size)
{
    ...
    FuzzTestSoundPoolSetLoop(data, size);
    return 0;
}