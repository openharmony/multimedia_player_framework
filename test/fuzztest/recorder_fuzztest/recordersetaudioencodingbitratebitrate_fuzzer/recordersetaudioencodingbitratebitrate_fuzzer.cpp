...
#include "recordersetaudioencodingbitratebitrate_fuzzer.h"
#include <cmath>
#include <iostream>
#include "aw_common.h"
#include "string_ex.h"
...
namespace OHOS {
namespace Media {
RecorderSetAudioEncodingBitRateBitRateFuzzer::RecorderSetAudioEncodingBitRateBitRateFuzzer()
{
}
...
bool RecorderSetAudioEncodingBitRateBitRateFuzzer::FuzzRecorderSetAudioEncodingBitRateBR(uint8_t *data, size_t size)
{
    ...
    g_videoRecorderConfig.outputFd = open("/data/test/media/recorder_SetAudioEncodingBitRateBitRate.mp4", O_RDWR);
    ...
    if (g_videoRecorderConfig.outputFd >= 0) {
        ...
        g_videoRecorderConfig.audioEncodingBitRate = *reinterpret_cast<int32_t *>(data);
...
extern "C" int LLVMFuzzerTestOneInput(uint8_t *data, size_t size)
{
    ...
    OHOS::FuzzTestRecorderSetAudioEncodingBitRateBitRate(data, size);
    return 0;
}