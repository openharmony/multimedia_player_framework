...
#include "recodersetwillmute_fuzzer.h"
#include <iostream>
#include <cmath>
#include "aw_common.h"
#include "string_ex.h"
...
namespace OHOS {
namespace Media {
RecorderSetWillMuteFuzzer::RecorderSetWillMuteFuzzer()
{
}
...
bool RecorderSetWillMuteFuzzer::FuzzRecorderSetWillMute(uint8_t *data, size_t size)
{
    ...
    if (g_videoRecorderConfig.outputFd >= 0) {
        ...
        g_baseFuzzSize = size;
        g_baseFuzzPos = 0;
        bool muteWhenInterrupted = GetData<bool>();
        TestRecorder::SetWillMuteWhenInterrupted(muteWhenInterrupted);
...
bool FuzzTestRecorderSetWillMute(uint8_t *data, size_t size)
{
    if (data == nullptr) {
        return true;
    }
    ...
    RecorderSetWillMuteFuzzer testRecorder;
    return testRecorder.FuzzRecorderSetWillMute(data, size);
}
}
...
extern "C" int LLVMFuzzerTestOneInput(uint8_t *data, size_t size)
{
    ...
    OHOS::FuzzTestRecorderSetWillMute(data, size);
    return 0;
}