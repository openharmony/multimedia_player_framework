...
#include "recoderiswatermarksupported_fuzzer.h"
#include <iostream>
#include <cmath>
#include "aw_common.h"
#include "string_ex.h"
...
namespace OHOS {
namespace Media {
RecorderIsWaterMarkSupportedFuzzer::RecorderIsWaterMarkSupportedFuzzer()
{
}
...
bool RecorderIsWaterMarkSupportedFuzzer::FuzzRecorderIsWaterMarkSupported(uint8_t *data, size_t size)
{
    ...
    if (g_videoRecorderConfig.outputFd >= 0) {
        ...
        g_baseFuzzSize = size;
        g_baseFuzzPos = 0;
        bool isWatermarkSupported = GetData<bool>();
        TestRecorder::IsWatermarkSupported(isWatermarkSupported);
...
bool FuzzTestRecorderIsWaterMarkSupported(uint8_t *data, size_t size)
{
    if (data == nullptr) {
        return true;
    }
    ...
    RecorderIsWaterMarkSupportedFuzzer testRecorder;
    return testRecorder.FuzzRecorderIsWaterMarkSupported(data, size);
}
}
...
extern "C" int LLVMFuzzerTestOneInput(uint8_t *data, size_t size)
{
    ...
    OHOS::FuzzTestRecorderIsWaterMarkSupported(data, size);
    return 0;
}