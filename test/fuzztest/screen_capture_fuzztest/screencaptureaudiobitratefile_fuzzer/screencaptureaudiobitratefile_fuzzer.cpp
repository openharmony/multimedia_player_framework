...
namespace {
constexpr OHOS::HiviewDFX::HiLogLabel LABEL = {LOG_CORE, LOG_DOMAIN, "ScreenCaptureAudioBitRateFileFuzzer"};
}
...
bool ScreenCaptureAudioBitRateFileFuzzer::FuzzScreenCaptureAudioBitRateFile(uint8_t *data, size_t size)
{
    ...
    int32_t outputFd = open((SCREEN_CAPTURE_ROOT + "screen_capture_fuzz_audiobitrate_file_01.mp4").c_str(),
        O_RDWR | O_CREAT, 0777);
...
extern "C" int LLVMFuzzerTestOneInput(uint8_t *data, size_t size)
{
    ...
    MEDIA_LOGI("FuzzTest ScreenCaptureAudioBitRateFileFuzzer size: %{public}d ", size);
