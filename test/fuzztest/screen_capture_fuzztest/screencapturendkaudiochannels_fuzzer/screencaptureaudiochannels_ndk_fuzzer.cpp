...
#include "screencaptureaudiochannels_ndk_fuzzer.h"
...
namespace OHOS {
namespace Media {
ScreenCaptureAudioChannelsNdkFuzzer::ScreenCaptureAudioChannelsNdkFuzzer()
{
}
...
bool ScreenCaptureAudioChannelsNdkFuzzer::FuzzScreenCaptureAudioChannelsNdk(uint8_t *data, size_t size)
{
    ...
    SetConfig(config);
    config.audioInfo.micCapInfo.audioChannels = fdp.ConsumeIntegral<uint32_t>();
...
bool FuzzTestScreenCaptureAudioChannelsNdk(uint8_t *data, size_t size)
{
    if (data == nullptr) {
        return true;
    }
    ...
    ScreenCaptureAudioChannelsNdkFuzzer testScreenCapture;
    return testScreenCapture.FuzzScreenCaptureAudioChannelsNdk(data, size);
}
} // namespace OHOS
...
extern "C" int LLVMFuzzerTestOneInput(uint8_t *data, size_t size)
{
    ...
    OHOS::FuzzTestScreenCaptureAudioChannelsNdk(data, size);
    return 0;
}