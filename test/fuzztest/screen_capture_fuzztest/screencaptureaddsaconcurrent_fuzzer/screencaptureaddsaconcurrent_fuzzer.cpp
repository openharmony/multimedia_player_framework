...
OHOS::Rect rect_;
std::shared_ptr<ScreenCaptureServer> screenCaptureServerPtr_;
ScreenCaptureServer *screenCaptureServer_ = nullptr;

extern "C" int LLVMFuzzerInitialize(int *argc, char ***argv)
{
    screenCaptureServerPtr_ = MakeScreenCaptureServerShared();
    if (!screenCaptureServerPtr_) {
        return 0;
    }
    screenCaptureServer_ = screenCaptureServerPtr_.get();
...
int FuzzScreenCaptureConconcurrentTestTwo(FuzzedDataProvider &provider, int code)
{
    ...
    switch (code) {
        ...
        case ACQUIRE_AUDIO_BUF: {
            ...
            AudioCaptureSourceType audioCaptureSourceType = static_cast<AudioCaptureSourceType>(
                provider.ConsumeIntegral<uint8_t>() % 4);
