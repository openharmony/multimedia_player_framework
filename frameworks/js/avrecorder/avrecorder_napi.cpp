    ...
    napi_value item, AVRecorderProfile profile)
{
    ...
    CHECK_AND_RETURN_RET(CommonNapi::GetPropertyBool(env, item, "hierarchicalPEnable", profile.hierarchicalPEnable),
        (asyncCtx->AVRecorderSignError(ret, "GethierarchicalPEnable", "hierarchicalPEnable"), ret));
...
void AVRecorderNapi::MediaProfileLog(bool isVideo, AVRecorderProfile &profile)
{
    if (isVideo) {
        MEDIA_LOGI("videoBitrate %{public}d, videoCodecFormat %{public}d, videoFrameWidth %{public}d,"
            " videoFrameHeight %{public}d, videoFrameRate %{public}d, isHdr %{public}d, hierarchicalPEnable %{public}d",
            profile.videoBitrate, profile.videoCodecFormat, profile.videoFrameWidth,
            profile.videoFrameHeight, profile.videoFrameRate, profile.isHdr, profile.hierarchicalPEnable);
        return;
    }
...
RetInfo AVRecorderNapi::SetProfile(std::shared_ptr<AVRecorderConfig> config)
{
    ...
    if (config->withVideo) {
        ...
        ret = recorder_->SetVideoHierarchicalPEnable(videoSourceID_, profile.hierarchicalPEnable);
        CHECK_AND_RETURN_RET(ret == MSERR_OK, GetRetInfo(ret, "SetVideoHierarchicalPEnable", "hierarchicalPEnable"));
    }
