    ...
    const std::string &add = "")
{
    ...
    if (errCode == MSERR_INVALID_VAL) {
        ...
        message = operate + MSErrorToExtErrorAPI9(static_cast<MediaServiceErrCode>(errCode)) + add;
    }
...
int32_t AVScreenCaptureNapi::GetWatermarkParam(std::unique_ptr<AVScreenCaptureAsyncContext> &asyncCtx,
...
std::shared_ptr<TaskHandler<RetInfo>> AVScreenCaptureNapi::AddWatermarkTask(
    const std::unique_ptr<AVScreenCaptureAsyncContext> &asyncCtx)
{
    return std::make_shared<TaskHandler<RetInfo>>([napi = asyncCtx->napi, &pixelMap = asyncCtx->pixelMap_,
        ...
        const std::string &option = AVScreenCapturegOpt::ADD_WATERMARK;
        MEDIA_LOGI("%{public}s Start", option.c_str());
        CHECK_AND_RETURN_RET(napi != nullptr, GetReturnInfo(MSERR_INVALID_OPERATION, option, ""));
        CHECK_AND_RETURN_RET(napi->screenCapture_ != nullptr, GetReturnInfo(MSERR_INVALID_OPERATION, option, ""));
        int32_t ret = napi->AddWatermark(pixelMap, watermarkConfig, watermarkCount);
        CHECK_AND_RETURN_RET_LOG(ret == MSERR_OK, GetReturnInfo(ret, "AddWatermarkTask", ""),
...
int32_t AVScreenCaptureNapi::GetWatermarkConfiguration(std::unique_ptr<AVScreenCaptureAsyncContext> &asyncCtx,
    napi_env env, napi_value args)
{
    CHECK_AND_RETURN_RET(CommonNapi::CheckValueType(env, args, napi_object),
        (asyncCtx->AVScreenCaptureSignError(MSERR_INVALID_VAL, "GetWatermarkConfiguration", "WatermarkConfig"),
        MSERR_INVALID_VAL));
    ...
    CHECK_AND_RETURN_RET(asyncCtx->watermarkConfiguration_,
        (asyncCtx->AVScreenCaptureSignError(MSERR_PARAMETER_VERIFICATION_FAILED, "GetWatermarkConfiguration",
        "WatermarkConfig"), MSERR_PARAMETER_VERIFICATION_FAILED));
    ...
    CHECK_AND_RETURN_RET(ret,
        (asyncCtx->AVScreenCaptureSignError(MSERR_PARAMETER_VERIFICATION_FAILED, "GetWatermarkConfiguration",
        "top", "config top cannot be null or less than zero"), MSERR_PARAMETER_VERIFICATION_FAILED));

    ret = CommonNapi::GetPropertyInt32(env, args, "left", config->left);
    CHECK_AND_RETURN_RET(ret,
        (asyncCtx->AVScreenCaptureSignError(MSERR_PARAMETER_VERIFICATION_FAILED, "GetWatermarkConfiguration",
        "left", "config left cannot be null or less than zero"), MSERR_PARAMETER_VERIFICATION_FAILED));
    ...
    if (CommonNapi::CheckhasNamedProperty(env, args, "width")) {
        ...
        CHECK_AND_RETURN_RET(ret && config->width > 0 && config->width <= WATERMARK_WIDTH_HEIGHT_MAX,
            (asyncCtx->AVScreenCaptureSignError(MSERR_PARAMETER_VERIFICATION_FAILED, "GetWatermarkConfiguration",
            "width", "config width must be greater than zero and less than or equal to 4096"),
    ...
    if (CommonNapi::CheckhasNamedProperty(env, args, "height")) {
        ...
        CHECK_AND_RETURN_RET(ret && config->height > 0 && config->height <= WATERMARK_WIDTH_HEIGHT_MAX,
            (asyncCtx->AVScreenCaptureSignError(MSERR_PARAMETER_VERIFICATION_FAILED, "GetWatermarkConfiguration",
            "height", "config height must be greater than zero and less than or equal to 4096"),
            MSERR_PARAMETER_VERIFICATION_FAILED));
    }
...
int32_t AVScreenCaptureNapi::GetWatermark(std::unique_ptr<AVScreenCaptureAsyncContext> &asyncCtx,
    napi_env env, napi_value args)
{
    CHECK_AND_RETURN_RET(CommonNapi::CheckValueType(env, args, napi_object),
        (asyncCtx->AVScreenCaptureSignError(MSERR_INVALID_VAL, "GetPixelMap", "PixelMap"), MSERR_INVALID_VAL));
...
int32_t AVScreenCaptureNapi::AddWatermark(std::shared_ptr<PixelMap> &pixelMap,
    std::shared_ptr<WatermarkConfiguration> &watermarkConfig, int32_t &watermarkCount)
{
    ...
    CHECK_AND_RETURN_RET_LOG(allocator != nullptr, MSERR_NO_MEMORY, "Failed to create allocator");
    ...
    CHECK_AND_RETURN_RET_LOG(buffer->memory_ != nullptr, MSERR_INVALID_VAL, "AVBuffer memory is null");
    CHECK_AND_RETURN_RET_LOG(buffer->meta_ != nullptr, MSERR_INVALID_VAL, "AVBuffer meta is null");
    ...
    return screenCapture_->AddWatermark(buffer, watermarkConfig->width, watermarkConfig->height, watermarkCount);
}

napi_value AVScreenCaptureNapi::JsAddWatermark(napi_env env, napi_callback_info info)
{
    MediaTrace trace("AVScreenCapture::JsAddWatermark");
    const std::string &opt = AVScreenCapturegOpt::ADD_WATERMARK;
    MEDIA_LOGI("Js %{public}s Start", opt.c_str());
    ...
    if (asyncCtx->napi->GetWatermarkParam(asyncCtx, env, args[0], args[1]) == MSERR_OK) {
        asyncCtx->task_ = AVScreenCaptureNapi::AddWatermarkTask(asyncCtx);
        (void)asyncCtx->napi->taskQue_->EnqueueTask(asyncCtx->task_);
        asyncCtx->opt_ = opt;
    }
    ...
    NAPI_CALL(env, napi_create_async_work(env, nullptr, resource, [](napi_env env, void* data) {
        ...
        if (asyncCtx->task_) {
            ...
            if ((result.Value().first == MSERR_EXT_API9_OK) &&
                (asyncCtx->opt_ == AVScreenCapturegOpt::ADD_WATERMARK)) {
