...
ANI_EXPORT ani_status ANI_Constructor(ani_vm *vm, uint32_t *result)
{
    ...
    if (ANI_OK != ohos::multimedia::systemSoundManager::ANIRegister(env)) {
        std::cerr << "Error from ohos::multimedia::systemSoundManager::ANIRegister" << std::endl;
        status = ANI_ERROR;
    }
    if (ANI_OK != ringtonePlayer::ANIRegister(env)) {
        std::cerr << "Error from ringtonePlayer::ANIRegister" << std::endl;
        status = ANI_ERROR;
    }
