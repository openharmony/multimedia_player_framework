...
int32_t IStreamIDManager::Play(const std::shared_ptr<SoundParser> &soundParser,
...
{
    MEDIA_LOGI("PostProcessingOfStreamDoPlayFailed failed, streamID is %{public}d", streamID);
    for (int32_t i = 0; i < static_cast<int32_t>(playingStreamIDs_.size()); i++) {
        if (streamID == playingStreamIDs_[i]) {
            std::shared_ptr<AudioStream> stream = GetStreamByStreamID(streamID);
            CHECK_AND_RETURN_RET_LOG(stream != nullptr, "PostProcessingOfStreamDoPlayFailed, stream is nullptr");
            playingStreamIDs_.erase(playingStreamIDs_.begin() + i);
...
void StreamIDManagerWithSameSoundInterrupt::RemoveStreamByStreamID(int32_t soundID)
{
...
int32_t StreamIDManagerWithSameSoundInterrupt::ClearStreamIDInDeque(int32_t soundID, int32_t streamID)
{
    ...
    stream->Release();
    stream->SetStreamState(StreamState::RELEASED);
    RemoveStreamByStreamID(soundID);
    return MSERR_OK;
}
...
void StreamIDManagerWithNoInterrupt::RemoveStreamByStreamID(int32_t soundID)
{
...
int32_t StreamIDManagerWithNoInterrupt::SetPlay(int32_t soundID, int32_t streamID,
    const PlayParams &playParameters)
{
    MEDIA_LOGI("StreamIDManagerWithNoInterrupt start");
    ...
    const PlayParams &playParameters)
{
    ...
    std::shared_ptr<AudioStream> lastStream = GetStreamByStreamID(lastStreamID);
    if (lastStream == nullptr) {
        MEDIA_LOGI("SetPlayWithSameSoundInterrupt, lastStream is nullptr");
        playingStreamIDs_.pop_back();
        return MSERR_INVALID_VAL;
    }
...
int32_t StreamIDManager::DoPlay(int32_t streamID)
{
    ...
    if (interruptMode_ == InterruptMode::SAME_SOUND_INTERRUPT && stream->DoPlayWithSameSoundInterrupt() == MSERR_OK) {
        MEDIA_LOGI("StreamIDManager::DoPlayWithSameSoundInterrupt successfully, streamID is %{public}d", streamID);
        return MSERR_OK;
    }
    if (interruptMode_ == InterruptMode::NO_INTERRUPT && stream->DoPlayWithNoInterrupt() == MSERR_OK) {
        MEDIA_LOGI("StreamIDManager::DoPlayWithNoInterrupt successfully, streamID is %{public}d", streamID);
        return MSERR_OK;
    }
...
void StreamIDManager::SetInterruptMode(InterruptMode interruptMode)
{
    std::lock_guard lock(streamIDManagerLock_);
    MEDIA_LOGI("StreamIDManager::SetInterruptMode, current interruptMode is %{public}d, "
    ...
    const std::shared_ptr<SoundParser> &soundParser)
{
    ...
    std::shared_ptr<AudioStream> stream = std::make_shared<AudioStream>(soundParser->GetSoundTrackFormat(), soundID,
        streamID, streamStopThreadPool_);
    CHECK_AND_RETURN_RET_LOG(stream != nullptr, MSERR_INVALID_VAL, "failed to create stream");
    CHECK_AND_RETURN_RET_LOG(stream->SetPcmSharedMemory(soundParser->GetAudioSharedMemory(), pcmBufferSize),
        MSERR_INVALID_VAL, "SetPcmSharedMemory failed");
    stream->SetManager(weak_from_this());
...
void StreamIDManager::OnPlayFinished(int32_t streamID)
{
    ...
    std::shared_ptr<AudioStream> stream = GetStreamByStreamID(streamID);
    CHECK_AND_RETURN_LOG(stream != nullptr, "OnPlayFinished, stream is nullptr");
    if (stream->GetStreamState() == StreamState::PLAYING) {
        stream->SetStreamState(StreamState::STOPPED);
    }
