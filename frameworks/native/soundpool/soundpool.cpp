...
int32_t SoundPool::Stop(int32_t streamID)
{
    ...
    std::lock_guard lock(soundPoolLock_);
    MEDIA_LOGI("SoundPool::Stop, stream(%{public}d) will be stopped", streamID);
...
int32_t SoundPool::SetLoop(int32_t streamID, int32_t loop)
{
    ...
    MEDIA_LOGE("SoundPool::SetLoop, can not find the stream(%{public}d)", streamID);
    return MSERR_SOUNDPOOL_STREAM_NOT_FOUND;
}
...
int32_t SoundPool::SetPriority(int32_t streamID, int32_t priority)
{
    ...
    if (priority < MIN_STREAM_PRIORITY) {
        MEDIA_LOGE("SoundPool::SetPriority, priority is invalid, assign lowest priority by default");
