...
HWTEST_F(PlayerUnitTest, Player_Http_001, TestSize.Level2)
{
    ...
    ret = player_->PrepareAsync();
    if (ret == MSERR_OK) {
        PlayFunTest(HTTP_PLAY);
    }
}

/**
 * @tc.name  : Test Player Https
 * @tc.number: Player_Https_001
 * @tc.desc  : Test Player Https source
 */
HWTEST_F(PlayerUnitTest, Player_Https_001, TestSize.Level2)
{
    int32_t ret = player_->SetSource("https://vt1.doubanio.com/202202281701/" \
        "5dead3eff6d32d8a772fc848b198fe47/view/movie/M/402810465.mp4");
    EXPECT_EQ(MSERR_OK, ret);
    sptr<Surface> videoSurface = player_->GetVideoSurface();
    ASSERT_NE(nullptr, videoSurface);
    EXPECT_EQ(MSERR_OK, player_->SetVideoSurface(videoSurface));
    ret = player_->PrepareAsync();
    if (ret == MSERR_OK) {
        PlayFunTest(HTTPS_PLAY);
    }
}

/**
 * @tc.name  : Test Player hls
 * @tc.number: Player_Hls_001
 * @tc.desc  : Test Player hls source
 */
HWTEST_F(PlayerUnitTest, Player_Hls_001, TestSize.Level2)
{
    int32_t ret = player_->SetSource("http://playertest.longtailvideo.com/adaptive/bipbop/gear4/prog_index.m3u8");
    EXPECT_EQ(MSERR_OK, ret);
    sptr<Surface> videoSurface = player_->GetVideoSurface();
    ASSERT_NE(nullptr, videoSurface);
    EXPECT_EQ(MSERR_OK, player_->SetVideoSurface(videoSurface));
    ret = player_->PrepareAsync();
    if (ret == MSERR_OK) {
        PlayFunTest(HLS_PLAY);
    }
}

/**
 * @tc.name  : Test Player hls
 * @tc.number: Player_Hls_002
 * @tc.desc  : Test Player hls source
 */
HWTEST_F(PlayerUnitTest, Player_Hls_002, TestSize.Level2)
{
    int32_t ret = player_->SetSource("http://123.57.24.33:8000/05.hls/hls_variant/index.m3u8");
    EXPECT_EQ(MSERR_OK, ret);
    sptr<Surface> videoSurface = player_->GetVideoSurface();
    ASSERT_NE(nullptr, videoSurface);
    EXPECT_EQ(MSERR_OK, player_->SetVideoSurface(videoSurface));
...
/**
 * @tc.name  : Test Player hls
 * @tc.number: Player_Hls_003
 * @tc.desc  : Test Player hls source
 */
HWTEST_F(PlayerUnitTest, Player_Hls_003, TestSize.Level2)
{
    int32_t ret = player_->SetSource("http://123.57.24.33:8000/05.hls/hls_variant3/index.m3u8");
    EXPECT_EQ(MSERR_OK, ret);
    sptr<Surface> videoSurface = player_->GetVideoSurface();
    ASSERT_NE(nullptr, videoSurface);
    EXPECT_EQ(MSERR_OK, player_->SetVideoSurface(videoSurface));
