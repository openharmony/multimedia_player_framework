...
public:
    ...
    void TraceApiEvent(int errCode,
        const std::string& apiName, int64_t startTime, HiviewDFX::HiTraceId traceId = HiviewDFX::HiTraceId());

private:
    std::string GenerateTransId();
#ifdef SUPPORT_HIAPPEVENT
    void TraceApiEventAsync(int errCode,
        const std::string& apiName, int64_t startTime, HiviewDFX::HiTraceId traceId);
    ...
    std::unordered_map<std::string, int64_t> lastReportTime_;
#endif
};
