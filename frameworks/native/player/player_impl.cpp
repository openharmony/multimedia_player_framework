...
void PlayerImpl::HandleListStateInfo(PlayerStates state, bool &shouldUpdateState, int32_t &extra)
{
    if (state == PLAYER_PLAYBACK_COMPLETE) {
        ...
        CHECK_AND_RETURN_LOG(itemList_.size() > std::numeric_limits<int32_t>::max(),
...
int32_t PlayerImpl::SetSubtitleCbDfxStatus(bool isRegistered)
{
    ScopedTimer timer("SetSubtitleCbDfxStatus", OVERTIME_WARNING_MS);
    MEDIA_LOGD("PlayerImpl:0x%{public}06" PRIXPTR " SetSubtitleCbDfxStatus in, isRegistered is %{public}d",
        FAKE_POINTER(this), isRegistered);
    CHECK_AND_RETURN_RET_LOG(playerService_ != nullptr, MSERR_SERVICE_DIED, "player service does not exist.");
    int32_t ret = MSERR_OK;
    LISTENER(ret = playerService_->SetSubtitleCbDfxStatus(isRegistered),
        "SetSubtitleCbDfxStatus", false, TIME_OUT_SECOND);
    return ret;
}
