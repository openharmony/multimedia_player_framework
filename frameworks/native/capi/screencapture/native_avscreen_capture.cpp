...
const char *OH_MD_KEY_SCREEN_CAPTURE_CONTENT_RECT = "screen_capture_content_rect";
}
...
private:
        ...
        OHOS::sptr<OH_AVBuffer> &ohAvBuffer, int64_t &timestamp)
    {
        ...
        if (format != nullptr && rsRect.x >= 0) {
            int32_t rectData[4] = {rsRect.x, rsRect.y, rsRect.w, rsRect.h};
            bool res = false;
            res = OH_AVFormat_SetIntBuffer(format, OH_MD_KEY_SCREEN_CAPTURE_CONTENT_RECT, rectData, sizeof(rectData));
