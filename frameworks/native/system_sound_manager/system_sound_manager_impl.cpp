...
const int32_t TONE_NOT_SET_FLAG = 0;
...
enum ExtToneType : int32_t {
    ...
    EXT_TYPE_RINGTONE_ESIM_ONE = 64,
    EXT_TYPE_RINGTONE_ESIM_TWO = 128,
    EXT_TYPE_MESSAGETONE_ESIM_ONE = 256,
    EXT_TYPE_MESSAGETONE_ESIM_TWO = 512,
};
...
static const char PARAM_HAPTICS_SETTING_RINGTONE_ESIM0[] = "const.multimedia.haptics_ringtone_esim_card_0_haptics";
static const char PARAM_HAPTICS_SETTING_RINGTONE_ESIM1[] = "const.multimedia.haptics_ringtone_esim_card_1_haptics";
static const char PARAM_HAPTICS_SETTING_SHOT_ESIM0[] = "const.multimedia.haptics_system_tone_esim_card_0_haptics";
static const char PARAM_HAPTICS_SETTING_SHOT_ESIM1[] = "const.multimedia.haptics_system_tone_esim_card_1_haptics";
...
vector<string> COLUMNS = {{RINGTONE_COLUMN_TONE_ID}, {RINGTONE_COLUMN_DATA}, {RINGTONE_COLUMN_DISPLAY_NAME},
    ...
    {RINGTONE_COLUMN_ALARM_TONE_SOURCE_TYPE}, {RINGTONE_COLUMN_MIME_TYPE}, {RINGTONE_COLUMN_SIZE}};
...
std::string GetToneTypeColumnName(int32_t toneType)
{
    switch (toneType) {
        case TONE_TYPE_NOTIFICATION:
            return RINGTONE_COLUMN_NOTIFICATION_TONE_TYPE;
        case TONE_TYPE_ALARM:
            return RINGTONE_COLUMN_ALARM_TONE_TYPE;
        case TONE_TYPE_RINGTONE:
            return RINGTONE_COLUMN_RING_TONE_TYPE;
        case TONE_TYPE_SHOT:
            return RINGTONE_COLUMN_SHOT_TONE_TYPE;
        default:
            return "";
    }
}
...
std::string GetToneSourceTypeColumnName(int32_t toneType)
{
    switch (toneType) {
        case TONE_TYPE_NOTIFICATION:
            return RINGTONE_COLUMN_NOTIFICATION_TONE_SOURCE_TYPE;
        case TONE_TYPE_ALARM:
            return RINGTONE_COLUMN_ALARM_TONE_SOURCE_TYPE;
        case TONE_TYPE_RINGTONE:
            return RINGTONE_COLUMN_RING_TONE_SOURCE_TYPE;
        case TONE_TYPE_SHOT:
            return RINGTONE_COLUMN_SHOT_TONE_SOURCE_TYPE;
        default:
            return "";
    }
}
...
int32_t ConvertToneType2ToneCategory(int32_t toneType)
{
    switch(toneType) {
        ...
        case TONE_TYPE_SHOT:
            return TONE_CATEGORY_TEXT_MESSAGE;
        case TONE_TYPE_NOTIFICATION:
...
void SystemSoundManagerImpl::InitMap(void)
{
    ...
    defaultoneTypeMap_[RINGTONE_TYPE_ESIM_CARD_0] = DEFAULT_RING_TYPE_ESIM_CARD_1;
    defaultoneTypeMap_[RINGTONE_TYPE_ESIM_CARD_1] = DEFAULT_RING_TYPE_ESIM_CARD_2;
    ...
    defaultsystemTypeMap_[SYSTEM_TONE_TYPE_ESIM_CARD_0] = DEFAULT_SHOT_TYPE_ESIM_CARD_1;
    defaultsystemTypeMap_[SYSTEM_TONE_TYPE_ESIM_CARD_1] = DEFAULT_SHOT_TYPE_ESIM_CARD_2;
    ...
    toneHapticsQueryArgsMap_ = {
        ...
        {ToneHapticsType::CALL_ESIM_CARD_0, {SIMCARD_MODE_ESIM_1, TONE_SETTING_TYPE_RINGTONE}},
        {ToneHapticsType::CALL_ESIM_CARD_1, {SIMCARD_MODE_ESIM_2, TONE_SETTING_TYPE_RINGTONE}},
        {ToneHapticsType::TEXT_MESSAGE_SIM_CARD_0, {SIMCARD_MODE_1, TONE_SETTING_TYPE_SHOT}},
        {ToneHapticsType::TEXT_MESSAGE_SIM_CARD_1, {SIMCARD_MODE_2, TONE_SETTING_TYPE_SHOT}},
        {ToneHapticsType::TEXT_MESSAGE_ESIM_CARD_0, {SIMCARD_MODE_ESIM_1, TONE_SETTING_TYPE_SHOT}},
        {ToneHapticsType::TEXT_MESSAGE_ESIM_CARD_1, {SIMCARD_MODE_ESIM_2, TONE_SETTING_TYPE_SHOT}},
        {ToneHapticsType::NOTIFICATION, {SIMCARD_MODE_BOTH, TONE_SETTING_TYPE_NOTIFICATION}},
        {ToneHapticsType::ALARM, {SIMCARD_MODE_BOTH, TONE_SETTING_TYPE_ALARM}},
    };
...
std::string SystemSoundManagerImpl::BuildRingtoneLibraryUri(bool isProxy) const
{
    return isProxy ? RINGTONE_LIBRARY_PROXY_DATA_URI_TONE_FILES + "&user=" +
        std::to_string(SystemSoundManagerUtils::GetCurrentUserId()) : RINGTONE_PATH_URI;
}

std::string SystemSoundManagerImpl::BuildVibrateLibraryUri(bool isProxy) const
{
    return isProxy ? RINGTONE_LIBRARY_PROXY_DATA_URI_VIBATE_FILES + "&user=" +
    ...
    uint32_t targetToneType, int32_t toneType)
{
    ...
    CHECK_AND_RETURN_RET_LOG(databaseTool.isInitialized && databaseTool.dataShareHelper != nullptr, toneAttrs,
        "QueryToneAttrsByType: the database tool is not ready!");

    DataShare::DatashareBusinessError businessError;
    DataShare::DataSharePredicates queryPredicates;
    std::string typeColumnName = GetToneTypeColumnName(toneType);

    if (toneType == TONE_TYPE_RINGTONE || toneType == TONE_TYPE_SHOT) {
        queryPredicates.SetWhereClause(typeColumnName + " & ? != 0");
    } else {
        queryPredicates.SetWhereClause(typeColumnName + " = ? ");
    }
    queryPredicates.SetWhereArgs({to_string(targetToneType)});

    std::string ringtoneLibraryUri = BuildRingtoneLibraryUri(databaseTool.isProxy);
    ...
    if (asset != nullptr) {
        ...
        toneAttrs.SetFileName(asset->GetDisplayName());
        toneAttrs.SetCategory(asset->GetToneType());
        toneAttrs.SetMediaType(asset->GetMediaType() == RINGTONE_MEDIA_TYPE_VIDEO ?
            ToneMediaType::MEDIA_TYPE_VID : ToneMediaType::MEDIA_TYPE_AUD);
    }
    if (results != nullptr) {
        results->Close();
    }
...
int32_t SystemSoundManagerImpl::ClearToneType(std::shared_ptr<DataShare::DataShareHelper> dataShareHelper,
    int32_t toneType)
{
    int32_t result = 0;
    DataShare::DatashareBusinessError businessError;
    DataShare::DataSharePredicates queryPredicates;
    queryPredicates.SetWhereClause(GetToneTypeColumnName(toneType) + " = ? ");
    queryPredicates.SetWhereArgs({to_string(TONE_SET_FLAG)});

    auto resultSet = dataShareHelper->Query(RINGTONEURI, queryPredicates, COLUMNS, &businessError);
    auto results = make_unique<RingtoneFetchResult<RingtoneAsset>>(move(resultSet));
    CHECK_AND_RETURN_RET_LOG(results != nullptr, result, "ClearToneType: results is nullptr!");
    ...
    while (asset != nullptr) {
        ...
        updateValuesBucket.Put(GetToneTypeColumnName(toneType), TONE_NOT_SET_FLAG);
        updateValuesBucket.Put(GetToneSourceTypeColumnName(toneType), SOURCE_TYPE_INVALID);
...
int32_t SystemSoundManagerImpl::ClearBitFromToneTypeColumn(std::shared_ptr<DataShare::DataShareHelper> dataShareHelper,
    uint32_t setValue, int32_t toneType)
{
    ...
    DataShare::DataSharePredicates queryPredicates;
    std::string typeColumnName = GetToneTypeColumnName(toneType);
    std::string sourceTypeColumnName = GetToneSourceTypeColumnName(toneType);
    queryPredicates.SetWhereClause(typeColumnName + " & ? != 0 ");
    queryPredicates.SetWhereArgs({to_string(setValue)});

    auto resultSet = dataShareHelper->Query(RINGTONEURI, queryPredicates, COLUMNS, &businessError);
    auto results = make_unique<RingtoneFetchResult<RingtoneAsset>>(move(resultSet));
    CHECK_AND_RETURN_RET_LOG(results != nullptr, result, "ClearBitFromToneTypeColumn: results is nullptr!");

    unique_ptr<RingtoneAsset> asset = results->GetFirstObject();
    while (asset != nullptr) {
        uint32_t storedValue;
        if (typeColumnName == RINGTONE_COLUMN_RING_TONE_TYPE) {
            storedValue = static_cast<uint32_t>(asset->GetRingtoneType());
        } else {
            storedValue = static_cast<uint32_t>(asset->GetShottoneType());
        }
        uint32_t remaining = storedValue & ~setValue;
...
void SystemSoundManagerImpl::InitDefaultToneHapticsMap()
{
    ...
    ReadDefaultToneHaptics(PARAM_HAPTICS_SETTING_RINGTONE_ESIM0, ToneHapticsType::CALL_ESIM_CARD_0);
    ReadDefaultToneHaptics(PARAM_HAPTICS_SETTING_RINGTONE_ESIM1, ToneHapticsType::CALL_ESIM_CARD_1);
    ReadDefaultToneHaptics(PARAM_HAPTICS_SETTING_SHOT_ESIM0, ToneHapticsType::TEXT_MESSAGE_ESIM_CARD_0);
    ReadDefaultToneHaptics(PARAM_HAPTICS_SETTING_SHOT_ESIM1, ToneHapticsType::TEXT_MESSAGE_ESIM_CARD_1);
}
...
{
    uint32_t setValue = toneType == TONE_TYPE_RINGTONE ?
        RingtoneTypeToBitMask(static_cast<RingtoneType>(subType)) :
        SystemToneTypeToBitMask(static_cast<SystemToneType>(subType));
    ClearBitFromToneTypeColumn(dataShareHelper, setValue, toneType);
    uint32_t finalType = setValue | storedType;
    DataSharePredicates updatePredicates;
    DataShareValuesBucket updateValuesBucket;
    updatePredicates.SetWhereClause(RINGTONE_COLUMN_TONE_ID + " = ? ");
    updatePredicates.SetWhereArgs({to_string(toneId)});
    updateValuesBucket.Put(GetToneTypeColumnName(toneType), static_cast<int32_t>(finalType));
    updateValuesBucket.Put(GetToneSourceTypeColumnName(toneType), SOURCE_TYPE_CUSTOMISED);
...
{
    ClearToneType(dataShareHelper, toneType);
    DataSharePredicates updatePredicates;
    DataShareValuesBucket updateValuesBucket;
    updatePredicates.SetWhereClause(RINGTONE_COLUMN_TONE_ID + " = ? ");
    updatePredicates.SetWhereArgs({to_string(toneId)});
    updateValuesBucket.Put(GetToneTypeColumnName(toneType), TONE_SET_FLAG);
    updateValuesBucket.Put(GetToneSourceTypeColumnName(toneType), SOURCE_TYPE_CUSTOMISED);
    return dataShareHelper->Update(RINGTONEURI, updatePredicates, updateValuesBucket);
}
...
int32_t SystemSoundManagerImpl::QueryUriForErrorType(
    std::shared_ptr<DataShare::DataShareHelper> dataShareHelper, const std::string &uri)
{
    DataShare::DatashareBusinessError businessError;
    DataShare::DataSharePredicates uriOnlyPredicates;
    uriOnlyPredicates.EqualTo(RINGTONE_COLUMN_DATA, uri);
    auto resultSetByUri = dataShareHelper->Query(RINGTONEURI, uriOnlyPredicates, COLUMNS, &businessError);
    ...
    const std::string &uri, const SetToneUriParams &params)
{
    ...
    queryPredicates.EqualTo(RINGTONE_COLUMN_TONE_TYPE, params.toneType);
    ...
    uint32_t storedToneType = GetStoredToneType(ringtoneAsset, params.toneType);
    if (IsToneAlreadySet(ringtoneAsset, params.toneType, params.subType, storedToneType)) {
        MEDIA_LOGI("Tone already set, skip update. toneType %{public}d, type %{public}d",
            params.toneType, params.subType);
        results->Close();
        return SUCCESS;
    }
    ...
    SetExtRingtoneUri(uri, ringtoneAsset->GetTitle(), params.subType,
        params.extToneType, changedRows);
    return changedRows > 0 ? SUCCESS : ERROR;
}

int32_t SystemSoundManagerImpl::SetNoToneUri(std::shared_ptr<DataShare::DataShareHelper> dataShareHelper,
    const SetToneUriParams &params)
{
    MEDIA_LOGI("SetNoToneUri: toneType %{public}d, subType %{public}d",
        params.toneType, params.subType);
    switch (params.toneType) {
        case TONE_TYPE_RINGTONE:
            return ClearBitFromToneTypeColumn(dataShareHelper,
                RingtoneTypeToBitMask(static_cast<RingtoneType>(params.subType)), TONE_TYPE_RINGTONE);
        case TONE_TYPE_NOTIFICATION:
            if (params.subType == SYSTEM_TONE_TYPE_NOTIFICATION) {
                return ClearToneType(dataShareHelper, TONE_TYPE_NOTIFICATION);
            }
            return ClearBitFromToneTypeColumn(dataShareHelper, SystemToneTypeToBitMask(
                static_cast<SystemToneType>(params.subType)), TONE_TYPE_SHOT);
        case TONE_TYPE_ALARM:
            return ClearToneType(dataShareHelper, TONE_TYPE_ALARM);
        default:
            MEDIA_LOGE("SetNoToneUri: invalid toneType %{public}d", params.toneType);
            return ERROR;
    }
}
    ...
    RingtoneType ringtoneType)
{
    ...
    CHECK_AND_RETURN_RET_LOG(IsValidRingtoneType(ringtoneType), MSERR_INVALID_VAL, "Invalid ringtone type");
    ...
    int32_t result = SetToneUriInternal(dataShareHelper, uri, toneParams);
    dataShareHelper->Release();
    return result;
}
...
ToneAttrs SystemSoundManagerImpl::GetCurrentRingtoneAttribute(RingtoneType ringtoneType)
{
    ...
    if (!IsValidRingtoneType(ringtoneType)) {
    ...
    RingtoneType ringtoneType)
{
    ...
    CHECK_AND_RETURN_RET_LOG(IsValidRingtoneType(ringtoneType), nullptr, "invalid ringtone type");
...
{
    std::lock_guard<std::mutex> lock(playerMutex_);
    CHECK_AND_RETURN_RET_LOG(IsValidSystemToneType(systemToneType), nullptr, "invalid system tone type");
...
{
    std::lock_guard<std::mutex> lock(uriMutex_);
    CHECK_AND_RETURN_RET_LOG(IsValidSystemToneType(systemToneType), MSERR_INVALID_VAL, "Invalid system tone type");
    ...
    SetToneUriParams toneParams = {TONE_TYPE_NOTIFICATION, static_cast<int32_t>(systemToneType),
    ...
    SystemToneType systemToneType)
{
    ...
    CHECK_AND_RETURN_RET_LOG(IsValidSystemToneType(systemToneType), toneAttrs.GetUri(), "Invalid system tone type");

    bool isProxy = false;
    std::shared_ptr<DataShare::DataShareHelper> dataShareHelper;
    SystemSoundManagerUtils::CreateDataShareHelper(STORAGE_MANAGER_MANAGER_ID, isProxy, dataShareHelper);
    CHECK_AND_RETURN_RET_LOG(dataShareHelper != nullptr, toneAttrs.GetUri(),
...
ToneAttrs SystemSoundManagerImpl::GetSystemToneAttrs(const DatabaseTool &databaseTool, SystemToneType systemToneType)
{
    ...
    ToneAttrs toneAttrs = { "", "", "", CUSTOMISED, category};
    if (!databaseTool.isInitialized || databaseTool.dataShareHelper == nullptr) {
        MEDIA_LOGE("GetSystemToneAttrs: The database tool is not ready!");
        return toneAttrs;
    }
...
std::string SystemSoundManagerImpl::GetDefaultHapticsUri(const DatabaseTool &databaseTool)
{
    CHECK_AND_RETURN_RET_LOG(databaseTool.isInitialized && databaseTool.dataShareHelper != nullptr, "",
        "GetDefaultHapticsUri: The database tool is not ready!");
    DataShare::DatashareBusinessError businessError;
    DataShare::DataSharePredicates queryPredicates;
    queryPredicates.EqualTo(RINGTONE_COLUMN_MIME_TYPE, DEFAULT_HAPTIC_QUERY_FLAG);
    std::string queryUri = BuildRingtoneLibraryUri(databaseTool.isProxy);
...
std::vector<std::shared_ptr<ToneAttrs>> SystemSoundManagerImpl::GetToneAttrListInternal(
    const std::shared_ptr<AbilityRuntime::Context> &context, int32_t toneTypeQuery, int32_t category)
{
    ...
    std::shared_ptr<DataShare::DataShareHelper> dataShareHelper;
    SystemSoundManagerUtils::CreateDataShareHelper(STORAGE_MANAGER_MANAGER_ID, isProxy, dataShareHelper);
    CHECK_AND_RETURN_RET_LOG(dataShareHelper != nullptr, toneAttrsArray, "Create dataShare failed.");
    std::string queryUri = BuildRingtoneLibraryUri(isProxy);
    Uri QUERYURI(queryUri);
    DataShare::DatashareBusinessError businessError;
    DataShare::DataSharePredicates queryPredicates;
    queryPredicates.EqualTo(RINGTONE_COLUMN_TONE_TYPE, to_string(toneTypeQuery));
    ...
    CHECK_AND_RETURN_RET_LOG(resultSet != nullptr, toneAttrsArray, "query failed.");
    auto results = make_unique<RingtoneFetchResult<RingtoneAsset>>(move(resultSet));
    CHECK_AND_RETURN_RET_LOG(results != nullptr, toneAttrsArray, "query failed, ringtone library error.");
    unique_ptr<RingtoneAsset> ringtoneAsset = results->GetFirstObject();
    while (ringtoneAsset != nullptr) {
        ...
        toneAttrsArray.push_back(toneAttrs);
        ringtoneAsset = results->GetNextObject();
    }
    if (toneAttrsArray.empty()) {
        MEDIA_LOGE("GetToneAttrListInternal: no tone in the ringtone library! type %{public}d", toneTypeQuery);
    }
    if (results != nullptr) {
        results->Close();
    }
    dataShareHelper->Release();
    return toneAttrsArray;
}
...
{
    return GetToneAttrListInternal(context, TONE_TYPE_RINGTONE, TONE_CATEGORY_RINGTONE);
}
...
{
    std::lock_guard<std::mutex> lock(uriMutex_);
    CHECK_AND_RETURN_RET_LOG(IsValidSystemToneType(systemToneType),  nullptr, "Invalid systemtone type");
    bool isProxy = false;
    std::shared_ptr<DataShare::DataShareHelper> dataShareHelper;
    SystemSoundManagerUtils::CreateDataShareHelper(STORAGE_MANAGER_MANAGER_ID, isProxy, dataShareHelper);
    CHECK_AND_RETURN_RET_LOG(dataShareHelper != nullptr, nullptr, "Create dataShare failed.");
    ...
    const std::shared_ptr<AbilityRuntime::Context> &context, SystemToneType systemToneType)
{
    int32_t category = systemToneType == SYSTEM_TONE_TYPE_NOTIFICATION ?
        TONE_CATEGORY_NOTIFICATION : TONE_CATEGORY_TEXT_MESSAGE;
    return GetToneAttrListInternal(context, TONE_TYPE_NOTIFICATION, category);
}
    ...
    const std::string &uri)
{
    ...
    SetToneUriParams toneParams = {TONE_TYPE_ALARM, 0, TONE_TYPE_ALARM};
    if (uri == NO_RING_SOUND) {
        int32_t result = SetNoToneUri(dataShareHelper, toneParams);
        dataShareHelper->Release();
        return result >= 0 ? SUCCESS : ERROR;
...
std::vector<std::shared_ptr<ToneAttrs>> SystemSoundManagerImpl::GetAlarmToneAttrList
    (const std::shared_ptr<AbilityRuntime::Context> &context)
{
    return GetToneAttrListInternal(context, TONE_TYPE_ALARM, TONE_CATEGORY_ALARM);
}
...
{
    if (!databaseTool.isInitialized || databaseTool.dataShareHelper == nullptr) {
        MEDIA_LOGE("OpenToneUri: The database tool is not ready!");
        SendPlaybackFailedEvent(INVALID_DATASHARE);
        return ERROR;
    }
    ...
    if (results == nullptr) {
        ...
        MEDIA_LOGE("OpenToneUri: query failed, ringtone library error.");
        return ERROR;
    }
    unique_ptr<RingtoneAsset> ringtoneAsset = results->GetFirstObject();
    if (ringtoneAsset == nullptr) {
...
int32_t SystemSoundManagerImpl::OpenCustomToneUri(const std::string &customAudioUri, int32_t toneType)
{
    ...
    if (results == nullptr) {
        SendPlaybackFailedEvent(QUERY_FAILED);
        MEDIA_LOGE("OpenCustomToneUri: query failed, ringtone library error.");
        return ERROR;
    }
    unique_ptr<RingtoneAsset> ringtoneAsset = results->GetFirstObject();
    if (ringtoneAsset != nullptr) {
...
{
    AudioStandard::AudioRingerMode mode = ringerMode_.load();
    MEDIA_LOGI("GetRingerMode: %{public}d", mode);
    return mode;
}
...
bool SystemSoundManagerImpl::ConvertToRingtoneType(ToneHapticsType toneHapticsType, RingtoneType &ringtoneType)
{
    switch (toneHapticsType) {
        ...
        case ToneHapticsType::CALL_SIM_CARD_1 :
            ...
            return true;
        case ToneHapticsType::CALL_ESIM_CARD_0 :
            ringtoneType = RINGTONE_TYPE_ESIM_CARD_0;
            return true;
        case ToneHapticsType::CALL_ESIM_CARD_1 :
...
bool SystemSoundManagerImpl::ConvertToSystemToneType(ToneHapticsType toneHapticsType, SystemToneType &systemToneType)
{
    switch (toneHapticsType) {
        ...
        case ToneHapticsType::TEXT_MESSAGE_SIM_CARD_1 :
            ...
            return true;
        case ToneHapticsType::TEXT_MESSAGE_ESIM_CARD_0 :
            systemToneType = SYSTEM_TONE_TYPE_ESIM_CARD_0;
            return true;
        case ToneHapticsType::TEXT_MESSAGE_ESIM_CARD_1 :
...
{
    if (!databaseTool.isInitialized || databaseTool.dataShareHelper == nullptr) {
        MEDIA_LOGE("GetCurrentToneUri: The database tool is not ready!");
        return "";
    }
    ...
    if (ConvertToRingtoneType(toneHapticsType, ringtoneType)) {
        currentToneUri = GetRingtoneAttrs(databaseTool, ringtoneType).GetUri();
    ...
    const DatabaseTool &databaseTool, ToneHapticsType toneHapticsType)
{
    ...
    std::string ringtoneLibraryUri = BuildVibrateLibraryUri(databaseTool.isProxy);
    Uri queryUri(ringtoneLibraryUri);
...
{
#ifdef SUPPORT_VIBRATOR
    CHECK_AND_RETURN_RET_LOG(IsValidToneHapticsType(toneHapticsType), IO_ERROR, "Invalid tone haptics type");
...
#ifdef SUPPORT_VIBRATOR
    if (!databaseTool.isInitialized || databaseTool.dataShareHelper == nullptr) {
        MEDIA_LOGE("GetToneHapticsSettings: The database tool is not ready!");
        return IO_ERROR;
    }
    if (toneUri.empty() || !IsValidToneHapticsType(toneHapticsType)) {
...
{
#ifdef SUPPORT_VIBRATOR
    CHECK_AND_RETURN_RET_LOG(IsValidToneHapticsType(toneHapticsType), OPERATION_ERROR, "Invalid tone haptics type");
    ...
    const std::string &toneUri, ToneHapticsType toneHapticsType, const ToneHapticsSettings &settings)
{
    ...
    if (!databaseTool.isInitialized || databaseTool.dataShareHelper == nullptr ||
        toneUri.empty() || !IsValidToneHapticsType(toneHapticsType)) {
    ...
    bool isSynced, std::vector<std::shared_ptr<ToneHapticsAttrs>> &toneHapticsAttrsArray)
{
    ...
    std::string queryUri = BuildVibrateLibraryUri(isProxy);
    Uri QUERYURI(queryUri);
...
{
    if (!databaseTool.isInitialized || databaseTool.dataShareHelper == nullptr) {
        MEDIA_LOGE("GetHapticsUriByStyle: The database tool is not ready!");
        return "";
    }
    ...
    MEDIA_LOGI("GetHapticsUriByStyle: standardHapticsUri %{public}s, style %{public}d", standardHapticsUri.c_str(),
        hapticsStyle);
    std::string vibrateFilesUri = BuildVibrateLibraryUri(databaseTool.isProxy);
    ...
    const std::string &standardHapticsUri, HapticsStyle hapticsStyle, const std::string &vibrateFilesUri)
{
    ...
    if (resultsByDisplayName != nullptr) {
        resultsByDisplayName->Close();
    }
...
#ifdef SUPPORT_VIBRATOR
    if (!databaseTool.isInitialized || databaseTool.dataShareHelper == nullptr) {
        MEDIA_LOGE("GetGentleHapticsAttr: The database tool is not ready!");
        return IO_ERROR;
    }
...
{
    if (!databaseTool.isInitialized || databaseTool.dataShareHelper == nullptr) {
        MEDIA_LOGE("OpenAudioUri: The database tool is not ready!");
        SendPlaybackFailedEvent(INVALID_DATASHARE);
        return "";
    }
    ...
    if (results == nullptr) {
        SendPlaybackFailedEvent(QUERY_FAILED);
        MEDIA_LOGE("OpenAudioUri: query failed, ringtone library error.");
        return newAudioUri;
    }
    unique_ptr<RingtoneAsset> ringtoneAsset = results->GetFirstObject();
    if (ringtoneAsset == nullptr) {
...
std::string SystemSoundManagerImpl::OpenCustomAudioUri(const std::string &customAudioUri)
{
    ...
    if (results == nullptr) {
        SendPlaybackFailedEvent(QUERY_FAILED);
        MEDIA_LOGE("OpenCustomAudioUri: query failed, ringtone library error.");
        return newAudioUri;
    }
...
std::string SystemSoundManagerImpl::OpenHapticsUri(const DatabaseTool &databaseTool, const std::string &hapticsUri)
{
    if (!databaseTool.isInitialized || databaseTool.dataShareHelper == nullptr) {
        SendPlaybackFailedEvent(INVALID_DATASHARE);
        MEDIA_LOGE("OpenHapticsUri: The database tool is not ready!");
        return "";
    }
    ...
    auto results = make_unique<RingtoneFetchResult<VibrateAsset>>(move(resultSet));
    if (results == nullptr) {
        MEDIA_LOGE("OpenHapticsUri: query failed, ringtone library error.");
        SendPlaybackFailedEvent(QUERY_FAILED);
    ...
    off_t fileSize, std::string mimeType, int result)
{
    ...
    MEDIA_LOGI("SendCustomizedToneEvent: app name is %{public}s", appName.c_str());
    ...
    bean->Add("RESULT", static_cast<int32_t>(realResult));
    MEDIA_LOGI("SendCustomizedToneEvent: result is %{public}d(0 success; -1 error).", realResult);
...
std::vector<ToneInfo> SystemSoundManagerImpl::GetCurrentToneInfos()
{
    ...
    toneAttrs = GetRingtoneAttrs(databaseTool, RINGTONE_TYPE_ESIM_CARD_0);
    toneInfos.push_back({EXT_TYPE_RINGTONE_ESIM_ONE, toneAttrs.GetUri(), toneAttrs.GetTitle()});

    toneAttrs = GetRingtoneAttrs(databaseTool, RINGTONE_TYPE_ESIM_CARD_1);
    toneInfos.push_back({EXT_TYPE_RINGTONE_ESIM_TWO, toneAttrs.GetUri(), toneAttrs.GetTitle()});
    ...
    toneAttrs = GetSystemToneAttrs(databaseTool, SYSTEM_TONE_TYPE_ESIM_CARD_0);
    toneInfos.push_back({EXT_TYPE_MESSAGETONE_ESIM_ONE, toneAttrs.GetUri(), toneAttrs.GetTitle()});

    toneAttrs = GetSystemToneAttrs(databaseTool, SYSTEM_TONE_TYPE_ESIM_CARD_1);
    toneInfos.push_back({EXT_TYPE_MESSAGETONE_ESIM_TWO, toneAttrs.GetUri(), toneAttrs.GetTitle()});
    ...
    std::string &ringtoneUri)
{
    ...
    CHECK_AND_RETURN_RET_LOG(IsValidRingtoneType(ringtoneType), nullptr, "Invalid ringtoneType");
