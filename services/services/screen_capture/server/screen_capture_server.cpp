...
static std::shared_ptr<OHOS::Media::ScreenCaptureServer> GetScreenCaptureServerByIdWithLock(int32_t id)
{
    std::unique_lock<std::shared_mutex> lock(mutexServerMapRWGlobal_);
                                        ...
                                        OHOS::sptr<OHOS::Notification::NotificationButtonOption> buttonOption)
{
    ...
    std::shared_ptr<OHOS::Media::ScreenCaptureServer> server = GetScreenCaptureServerByIdWithLock(notificationId);
    if (server == nullptr) {
...
int32_t ScreenCaptureServer::StartHomeVideoCapture()
{
    ...
    MEDIA_LOGD("StartHomeVideoCapture producerSurface: %{public}" PRIu64, producerSurface->GetUniqueId());
    ...
    MEDIA_LOGI("ScreenCaptureServer:StartHomeVideoCapture surfaceCb_: 0x%{public}06" PRIXPTR,
        FAKE_POINTER(surfaceCb_.GetRefPtr()));
    (static_cast<ScreenCapBufferConsumerListener*>(surfaceCb_.GetRefPtr()))->StartBufferThread();
    return MSERR_OK;
}
