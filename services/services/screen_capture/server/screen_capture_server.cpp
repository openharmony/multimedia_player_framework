...
{
    if (screenCaptureCb_ != nullptr && Can(CAP_ALIVE)) {
...
int32_t ScreenCaptureServer::ReportAVScreenCaptureUserChoice(int32_t sessionId, const std::string &content)
{
    ...
    if (server->IsPickerPopUp() && server->isPresentPickerPopWindow_ && server->Can(CAP_RUNNING)) {
        return server->HandlePresentPickerWindowCase(root, content);
    }
#endif
    if (server->Can(CAP_POPUP)) {
        return server->HandlePopupWindowCase(root, content);
    }
    CHECK_AND_RETURN_RET(server->GetSCServerDataType() != DataType::ORIGINAL_STREAM || !server->Can(CAP_RUNNING),
...
DMError ScreenCaptureServer::CreateMirror(const std::vector<uint64_t> &displayIds, std::vector<ScreenId> &mirrorIds)
{
    ...
    } else if (Can(CAP_PAUSED) && !canvasRotation_) {
...
bool ScreenCaptureServer::Can(uint32_t cap) const
{
...
int32_t ScreenCaptureServer::SetCaptureMode(CaptureMode captureMode)
{
    ...
    std::lock_guard<std::mutex> lock(mutex_);
    CHECK_AND_RETURN_RET_LOG(Can(CAP_CONFIG), MSERR_INVALID_OPERATION,
...
int32_t ScreenCaptureServer::StartFileAudioCapture()
{
    MEDIA_LOGI("ScreenCaptureServer: 0x%{public}06" PRIXPTR " StartFileAudioCapture start.", FAKE_POINTER(this));
    if (isMicrophoneSwitchTurnOn_) {
        int32_t ret = StartFileMicAudioCapture();
        TRUE_LOG(ret != MSERR_OK, MEDIA_LOGE, "StartFileAudioCapture StartFileMicAudioCapture failed %{public}d", ret);
    }
    int32_t ret = StartFileInnerAudioCapture();
    CHECK_AND_RETURN_RET_LOG(ret == MSERR_OK, ret,
        "StartFileAudioCapture StartFileInnerAudioCapture failed, ret:%{public}d", ret);
    MEDIA_LOGI("ScreenCaptureServer: 0x%{public}06" PRIXPTR " StartFileAudioCapture OK.", FAKE_POINTER(this));
    return MSERR_OK;
}
...
int32_t ScreenCaptureServer::StartScreenCaptureFile()
{
    ...
    CHECK_AND_RETURN_RET_LOG(ret == MSERR_OK, ret, "StartFileInnerAudioCapture failed, ret:%{public}d, "
        "dataType:%{public}d", ret, captureConfig_.dataType);
...
int32_t ScreenCaptureServer::StartScreenCapture(bool isPrivacyAuthorityEnabled)
{
    ...
    CHECK_AND_RETURN_RET_LOG(Can(CAP_INIT), MSERR_INVALID_OPERATION,
...
int32_t ScreenCaptureServer::AcquireAudioBuffer(std::shared_ptr<AudioBuffer> &audioBuffer, AudioCaptureSourceType type)
{
    ...
    CHECK_AND_RETURN_RET_LOG(Can(CAP_ACTIVE), MSERR_INVALID_OPERATION,
...
{
    std::unique_lock<std::mutex> lock(mutex_);
    CHECK_AND_RETURN_RET_LOG(Can(CAP_ALIVE), MSERR_INVALID_OPERATION, "ExcludeContent failed, cannot save config");
...
{
    if (!captureConfig_.strategy.keepCaptureDuringCall && isInTelCall && !Can(CAP_PAUSED)) {
...
int32_t ScreenCaptureServer::SetCanvasRotation(bool canvasRotation)
{
    ...
    CHECK_AND_RETURN_RET(Can(CAP_ACTIVE), MSERR_OK);  // Before Start
...
int32_t ScreenCaptureServer::StopScreenCaptureInner(AVScreenCaptureStateCode stateCode)
{
    ...
    if (Can(CAP_ALIVE) && !Can(CAP_ACTIVE)) {
        StopNotStartedScreenCapture(stateCode);
        return MSERR_OK;
    }
    CHECK_AND_RETURN_RET(Can(CAP_ALIVE), MSERR_OK);
    ...
    CHECK_AND_RETURN_RET_LOG(Can(CAP_ACTIVE), ret, "state:%{public}d", captureState_.load());
...
{
    if (!Can(CAP_POPUP) && !isPresentPickerPopWindow_) {
    ...
    const std::vector<std::shared_ptr<AudioRendererChangeInfo>> &audioRendererChangeInfos)
{
    ...
    CHECK_AND_RETURN(screenCaptureServer->Can(CAP_ALIVE));
...
int32_t ScreenCaptureServer::PauseScreenCaptureInner(AVScreenCaptureStateCode stateCode)
{
    ...
    CHECK_AND_RETURN_RET_LOG(Can(CAP_RUNNING), MSERR_INVALID_OPERATION,
    ...
    CHECK_AND_RETURN_RET_LOG(captureConfig_.strategy.enablePause,
        MSERR_INVALID_OPERATION, "PauseScreenCaptureInner failed, enablePause is false");
...
int32_t ScreenCaptureServer::ResumeScreenCaptureInner(AVScreenCaptureStateCode stateCode)
{
    ...
    MEDIA_LOGI("0x%{public}06" PRIXPTR " Instances ResumeScreenCaptureInner, state:%{public}d, stateCode:%{public}d",
        FAKE_POINTER(this), captureState_.load(), static_cast<int32_t>(stateCode));
    CHECK_AND_RETURN_RET_LOG(Can(CAP_PAUSED), MSERR_INVALID_OPERATION,
        "ResumeScreenCaptureInner failed, cannot resume in current state, state:%{public}d", captureState_.load());
    CHECK_AND_RETURN_RET_LOG(captureConfig_.strategy.enablePause,
        MSERR_INVALID_OPERATION, "ResumeScreenCaptureInner failed, enablePause is false");
...
int32_t ScreenCaptureServer::PauseRecorder()
{
    ...
    if (recorder_) {
        ...
        CHECK_AND_RETURN_RET_LOG(ret == MSERR_OK, ret, "ScreenCaptureServer: 0x%{public}06" PRIXPTR
            "PauseRecorder failed %{public}d", FAKE_POINTER(this), ret);
    }
