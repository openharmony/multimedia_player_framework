...
int32_t ScreenCaptureServer::StartInnerAudioCapture()
{
    ...
    if (captureConfig_.audioInfo.innerCapInfo.state == AVScreenCaptureParamValidationState::VALIDATION_VALID) {
        ...
        if (speakerAliveStatus_ && isMicrophoneOn_) {
    ...
    std::shared_ptr<AudioBuffer> &micAudioBuffer, AVScreenCaptureMixMode type)
{
    ...
    if (type == AVScreenCaptureMixMode::MIX_MODE && micAudioCapture_ != nullptr &&
        ...
        if (isInnerAudioCaptureWorking_ && innerAudioCapture_->AcquireAudioBuffer(innerAudioBuffer) != MSERR_OK) {
            MEDIA_LOGE("innerAudioCapture AcquireAudioBuffer failed");
        }
        if (isMicAudioCaptureWorking_ && micAudioCapture_->AcquireAudioBuffer(micAudioBuffer) != MSERR_OK) {
...
int32_t ScreenCaptureServer::ReleaseAudioBufferMix(AVScreenCaptureMixMode type)
{
    ...
    if (type == AVScreenCaptureMixMode::MIX_MODE && micAudioCapture_ != nullptr &&
        ...
        if (isInnerAudioCaptureWorking_) {
            ...
            CHECK_AND_RETURN_RET_LOG(retInner == MSERR_OK, retInner, "release inner audio buffer failed");
        }
        if (isMicAudioCaptureWorking_) {
            int32_t retMic = micAudioCapture_->ReleaseAudioBuffer();
            CHECK_AND_RETURN_RET_LOG(retMic == MSERR_OK, retMic, "release mic audio buffer failed");
        }
        return MSERR_OK;
    }
    if (type == AVScreenCaptureMixMode::MIX_MODE && innerAudioCapture_ != nullptr && isInnerAudioCaptureWorking_) {
...
int32_t ScreenCaptureServer::SetMicrophoneEnabled(bool isMicrophone)
{
    ...
    if (captureState_ == AVScreenCaptureState::STARTED) {
        if (isMicrophone) {
            ...
            if (isInnerAudioCaptureWorking_ && innerAudioCapture_ && speakerAliveStatus_) {
                isInnerAudioCaptureWorking_ = false;
