...
int32_t RecorderServer::SetVideoHierarchicalPEnable(int32_t sourceId, bool hierarchicalPEnable)
{
    std::lock_guard<std::mutex> lock(mutex_);
    CHECK_STATUS_FAILED_AND_LOGE_RET(status_ != REC_CONFIGURED, MSERR_INVALID_OPERATION);
    CHECK_AND_RETURN_RET_LOG(recorderEngine_ != nullptr, MSERR_NO_MEMORY, "engine is nullptr");
    config_.hierarchicalPEnable = hierarchicalPEnable;
    VidHierarchicalPEnable vidHierarchicalPEnable(hierarchicalPEnable);
    auto task = std::make_shared<TaskHandler<int32_t>>([&, this] {
        return recorderEngine_->Configure(sourceId, vidHierarchicalPEnable);
    });
    int32_t ret = taskQue_.EnqueueTask(task);
    CHECK_AND_RETURN_RET_LOG(ret == MSERR_OK, ret, "EnqueueTask failed");
...
int32_t RecorderServer::DumpInfo(int32_t fd)
{
    ...
    dumpString += "RecorderServer hierarchicalPEnable is: " + std::to_string(config_.hierarchicalPEnable) + "\n";
