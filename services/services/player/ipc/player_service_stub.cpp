...
void PlayerServiceStub::FillPlayerFuncPart2()
{
    ...
    playerFuncs_[GET_LIVES_CURRENT_TIME] = { "GetLiveStreamCurrentTime",
        [this](MessageParcel &data, MessageParcel &reply) { return GetLiveStreamCurrentTime(data, reply); } };
}
...
int32_t PlayerServiceStub::GetLiveStreamCurrentTime(int32_t &currentTime)
{
    MediaTrace trace("PlayerServiceStub::GetLiveStreamCurrentTime");
    CHECK_AND_RETURN_RET_LOG(playerServer_ != nullptr, MSERR_NO_MEMORY, "player server is nullptr");
    return playerServer_->GetLiveStreamCurrentTime(currentTime);
}
...
int32_t PlayerServiceStub::GetLiveStreamCurrentTime(MessageParcel &data, MessageParcel &reply)
{
    (void)data;
    int32_t currentTime = -1;
    int32_t ret = GetLiveStreamCurrentTime(currentTime);
    reply.WriteInt32(currentTime);
    reply.WriteInt32(ret);
    return MSERR_OK;
}
