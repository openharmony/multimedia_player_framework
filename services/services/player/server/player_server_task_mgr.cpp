    ...
    const std::string &taskName, const std::shared_ptr<ITaskHandler> &cancelTask)
{
    ...
    if (taskName.compare("volume") == 0) {
        ...
        MEDIA_LOGI("0x%{public}06" PRIXPTR " task[%{public}s] is in processing, the new task[%{public}s], num:%{public}u",
            FAKE_POINTER(this), currTwoPhaseTaskName_.c_str(), taskName.c_str(), pendingTwoPhaseTasks_.size());
    }
