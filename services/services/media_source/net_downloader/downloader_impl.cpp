...
namespace {
constexpr HiLogLabel LABEL = {LOG_CORE, LOG_DOMAIN_PLAYER, "DownloaderImpl"};
constexpr int32_t MIN_URL_LENGTH = 10;
constexpr int32_t MIN_PATH_LENGTH = 1;
...
int32_t DownloaderImpl::Start()
{
    ...
    messageQueue_->Start([this](const Message &msg) {
    ...
    task_ = std::make_shared<DownloadTask>(taskId_, url_, outputPath_, header_, config_, this);
    
    int32_t ret = task_->Start();
    if (ret != DOWNLOAD_ERROR_OK) {
        state_.store(DOWNLOAD_FAILED);
        NotifyStateChanged(DOWNLOAD_FAILED);
        NotifyFailed(DOWNLOAD_ERROR_INTERNAL, ret, "Failed to start download task");
        task_ = nullptr;
        return ret;
    }
...
int32_t DownloaderImpl::Resume()
{
    ...
    if (ret != DOWNLOAD_ERROR_OK) {
        state_.store(DOWNLOAD_FAILED);
        NotifyStateChanged(DOWNLOAD_FAILED);
        NotifyFailed(DOWNLOAD_ERROR_INTERNAL, ret, "Failed to resume download task");
        return ret;
    }
...
int32_t DownloaderImpl::GetProgress(DownloadProgress &progress)
{
    ...
    if (task_ != nullptr && currentState == DOWNLOAD_RUNNING) {
...
void DownloaderImpl::OnTaskStateChanged(DownloadState state)
{
    state_.store(state);
    NotifyStateChanged(state);
}

void DownloaderImpl::OnTaskCompleted(int64_t downloadedSize)
{
    progress_.downloadedSize = downloadedSize;
    if (progress_.totalSize > 0) {
        progress_.progressPercent = static_cast<int32_t>(downloadedSize * 100 / progress_.totalSize);
    }
    NotifyCompleted(downloadedSize);
}

void DownloaderImpl::OnTaskFailed(DownloadErrorType errorType, int32_t errorCode, const std::string &errorMsg)
{
    NotifyFailed(errorType, errorCode, errorMsg);
}

void DownloaderImpl::OnTaskProgress(const DownloadProgress &progress)
{
    progress_ = progress;
