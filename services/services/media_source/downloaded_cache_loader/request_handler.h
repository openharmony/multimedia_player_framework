...
namespace OHOS {
namespace Media {
class DownloadedRequestHandler : public std::enable_shared_from_this<DownloadedRequestHandler> {
public:
    explicit DownloadedRequestHandler(std::shared_ptr<Plugins::HttpPlugin::NetworkClient> client);
    ...
    void SetCacheManager(std::shared_ptr<DownloadedCacheManager> cacheManager);
    void SetDownloadedFileCacheManager(std::shared_ptr<DownloadedFileCacheManager> DownloadedFileCacheManager);
    void SetHeaderParser(std::shared_ptr<DownloadedHttpHeaderParser> parser);
