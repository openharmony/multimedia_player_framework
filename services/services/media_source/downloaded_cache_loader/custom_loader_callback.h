...
namespace OHOS {
namespace Media {
class DownloadedCacheLoaderCallback : public LoaderCallback, public std::enable_shared_from_this<DownloadedCacheLoaderCallback> {
public:
    DownloadedCacheLoaderCallback(int64_t uuid, const std::shared_ptr<LoadingRequest>& request,
        const std::shared_ptr<Task>& task, const std::shared_ptr<Task>& readTask,
        const std::shared_ptr<Task>& interruptedTask);
    
    ~DownloadedCacheLoaderCallback();
    ...
    void SubmitNonRandomReadJob(int64_t uuid, int64_t requestedOffset,
        int64_t requestedLength, const std::weak_ptr<DownloadedCacheLoaderCallback>& weakThis);
    void HandleRequestDataCore(const std::shared_ptr<DownloadedCacheLoaderCallback>& self,
        int64_t uuid, int64_t requestedOffset, int64_t requestedLength);
    void HandleNonCacheRequest(const std::shared_ptr<DownloadedCacheLoaderCallback>& self,
        int64_t uuid, int64_t requestedOffset, int64_t requestedLength);
    void HandleCacheRequest(const std::shared_ptr<DownloadedCacheLoaderCallback>& self,
        int64_t uuid, int64_t requestedOffset, int64_t requestedLength);
    bool ProcessShard(const std::shared_ptr<DownloadedCacheLoaderCallback>& self, int64_t uuid,
    ...
    static void AppendToBuffer(DownloadedCacheLoaderCallback* downloader, const uint8_t* buffer, int64_t dataLen);
...
private:
    ...
    void StartDownloadTask(const std::weak_ptr<DownloadedCacheLoaderCallback>& weakThis);
    void ReadDataTask(const std::weak_ptr<DownloadedCacheLoaderCallback>& weakThis,
        int64_t uuid, int64_t requestedLength);
    void HandleInitialization(int64_t& len, bool& noSizeAndRequest, const std::shared_ptr<DownloadedCacheLoaderCallback>& self);
    ...
    void RespondDataChunk(int64_t start, int64_t& read, int64_t& len, bool noSizeAndRequest,
        const std::shared_ptr<DownloadedCacheLoaderCallback>& self);
    void Redownload(const std::weak_ptr<DownloadedCacheLoaderCallback>& weakThis);
    ...
    std::shared_ptr<DownloadedFileCacheManager> DownloadedFileCacheManager_;
    std::shared_ptr<DownloadedHttpHeaderParser> headerParser_;
    std::shared_ptr<DownloadedRequestHandler> DownloadedRequestHandler_;
