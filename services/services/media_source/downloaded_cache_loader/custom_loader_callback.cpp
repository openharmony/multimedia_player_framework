...
namespace Media {
namespace {
    constexpr OHOS::HiviewDFX::HiLogLabel LABEL = { LOG_CORE, LOG_DOMAIN_SYSTEM_PLAYER, "DownloadedCacheLoaderCallback" };
...
DownloadedCacheLoaderCallback::DownloadedCacheLoaderCallback(int64_t uuid,
    ...
    : uuid_(uuid), request_(request), task_(task), readTask_(readTask), interruptedTask_(interruptedTask),
      ...
      DownloadedFileCacheManager_(DownloadedFileCacheManager::Create()),
      headerParser_(std::make_shared<DownloadedHttpHeaderParser>()),
      DownloadedRequestHandler_(std::make_shared<DownloadedRequestHandler>(nullptr)),
      isFirstCallback_(false),
      isHeaderResponded_(false),
      isSupportLocalCache_(true),
      isClosed_(false) {
    DownloadedRequestHandler_->SetCacheManager(cacheManager_);
    DownloadedRequestHandler_->SetDownloadedFileCacheManager(DownloadedFileCacheManager_);
    DownloadedRequestHandler_->SetHeaderParser(headerParser_);
    buffer_ = static_cast<uint8_t *>(malloc(SHARD_SIZE));
    currentBuffer_ = static_cast<uint8_t *>(malloc(SHARD_SIZE / 4));
    FALSE_RETURN_MSG(buffer_ != nullptr, "buffer_ is nullptr");
    FALSE_RETURN_MSG(currentBuffer_ != nullptr, "currentBuffer_ is nullptr");
    DownloadedNetworkClientAgent::Create();
}

DownloadedCacheLoaderCallback::~DownloadedCacheLoaderCallback()
{
    if (DownloadedRequestHandler_->GetClient()) {
        DownloadedRequestHandler_->GetClient()->Deinit();
        DownloadedRequestHandler_->GetClient()->Close(false);
        DownloadedRequestHandler_->SetClient(nullptr);
    }
    if (buffer_) {
        free(buffer_);
        buffer_ = nullptr;
...
int64_t DownloadedCacheLoaderCallback::Open(std::shared_ptr<LoadingRequest>& request)
{
    ...
    if (urlStr.empty()) {
        ...
        headerMap_ = DownloadedRequestHandler_->GetHeaders();
        CreateUrlDir();
        return uuid_;
    }
    ...
    DownloadedHttpHeaderParser::ParseHttpHeader(headerMap_, content);
    CreateUrlDir();
    return uuid_;
}

void DownloadedCacheLoaderCallback::CreateUrlDir()
{
    if (DownloadedRequestHandler_->IsRequestError()) {
    ...
    if (contentLength != headerMap_.end()) {
        FALSE_RETURN_MSG(StrToLong(contentLength->second, size_), "Failed to read size.");
    }
    ...
    DownloadedRequestHandler_->SetUrlDir(urlDir_);
    MEDIA_LOG_I("RxBodyData first come randomAccess_ %{public}d", randomAccess_);
}

void DownloadedCacheLoaderCallback::Read(int64_t uuid, int64_t requestedOffset, int64_t requestedLength)
{
    ...
    if (!DownloadedRequestHandler_->GetClient()) {
        MEDIA_LOG_I("Read DownloadedRequestHandler_->GetClient() is null");
        auto client = DownloadedNetworkClientAgent::NewInstance(&RxHeaderData, &RxBodyData, this);
        FALSE_RETURN_MSG(client != nullptr, "client is null");
        client->Init();
        DownloadedRequestHandler_->SetClient(client);
    }
...
void DownloadedCacheLoaderCallback::HandleNonRandomAccessLogic(int64_t uuid, int64_t requestedOffset,
    int64_t requestedLength, const std::weak_ptr<DownloadedCacheLoaderCallback>& weakThis)
{
    ...
    DownloadedRequestHandler_->SetPath(path_);
    ...
    task_->SubmitJobOnce([weakThis, requestedOffset, requestedLength] {
        ...
        if (self->size_ != -1 && self->DownloadedFileCacheManager_->GetSize(dataPath) != self->size_) {
            self->DownloadedFileCacheManager_->Clear(dataPath);
        }
...
void DownloadedCacheLoaderCallback::SubmitNonRandomReadJob(int64_t uuid, int64_t requestedOffset,
    int64_t requestedLength, const std::weak_ptr<DownloadedCacheLoaderCallback>& weakThis)
{
    std::function<void()> job = [weakThis, uuid, requestedOffset, requestedLength] {
        ...
        int64_t fileSize = self->DownloadedFileCacheManager_->GetSize(dataPath);
        int64_t readLength = (requestedLength == -1) ? fileSize : requestedLength;

        while (fileSize < requestedOffset + readLength) {
            usleep(WAIT_TIME_US);
            fileSize = self->DownloadedFileCacheManager_->GetSize(dataPath);
        }
        ...
        if (self->DownloadedFileCacheManager_->Read(dataPath, buffer->GetBase(), requestedOffset, readLength) != 0) {
...
void DownloadedCacheLoaderCallback::HandleRequestDataCore(const std::shared_ptr<DownloadedCacheLoaderCallback>& self,
...
void DownloadedCacheLoaderCallback::HandleNonCacheRequest(const std::shared_ptr<DownloadedCacheLoaderCallback>& self,
    int64_t uuid, int64_t requestedOffset, int64_t requestedLength)
{
    if (!self) {
        return;
...
void DownloadedCacheLoaderCallback::HandleCacheRequest(const std::shared_ptr<DownloadedCacheLoaderCallback>& self,
    int64_t uuid, int64_t requestedOffset, int64_t requestedLength)
{
    if (!self) {
        return;
...
bool DownloadedCacheLoaderCallback::ProcessShard(const std::shared_ptr<DownloadedCacheLoaderCallback>& self, int64_t uuid,
    int index, int startIndex, int endIndex)
{
    ...
    self->DownloadedRequestHandler_->SetPath(self->path_);
    std::string dataPath = self->urlDir_ + "/" + self->path_ + ".data";
    int len = (index == (self->size_ - 1) / SHARD_SIZE) ? (self->size_ % SHARD_SIZE) : SHARD_SIZE;
    if (!self->DownloadedFileCacheManager_->IsValid(dataPath, len)) {
    ...
    if (self->DownloadedFileCacheManager_->Read(dataPath, buffer->GetBase(), fileBeg, fileReadLen) != 0) {
        self->request_->FinishLoading(uuid, 1);
        return false;
    }
...
void DownloadedCacheLoaderCallback::Close(int64_t uuid)
{
    (void)uuid;
    std::lock_guard<std::mutex> lock(mutex_);
    isClosed_.store(true);
    auto client = DownloadedRequestHandler_->GetClient();
...
void DownloadedCacheLoaderCallback::SetUrl(std::string url)
{
    url_ = url;
}

void DownloadedCacheLoaderCallback::RequestData(const std::shared_ptr<LoadingRequest>& request, int64_t start, int64_t length)
{
    if (!DownloadedRequestHandler_->GetClient()) {
        MEDIA_LOG_I("RequestData DownloadedRequestHandler GetClient is null");
        auto client = DownloadedNetworkClientAgent::NewInstance(&RxHeaderData, &RxBodyData, this);
        FALSE_RETURN_MSG(client != nullptr, "client is null");
        client->Init();
        DownloadedRequestHandler_->SetClient(client);
    }
    DownloadedRequestHandler_->SetUuid(uuid_);
    DownloadedRequestHandler_->Request(start, length, request->GetUrl(), request->GetHeader(), request);
}

size_t DownloadedCacheLoaderCallback::RxHeaderData(void* buffer, size_t size, size_t nitems, void* userParam)
{
    FALSE_RETURN_V_MSG_E(buffer != nullptr, 0, "buffer is nullptr");
    FALSE_RETURN_V_MSG_E(userParam != nullptr, 0, "userParam is nullptr");
    auto mediaDownloader = static_cast<DownloadedCacheLoaderCallback*>(userParam);
    mediaDownloader->DownloadedRequestHandler_->OnHeaderReceived(buffer, size, nitems);
    return size * nitems;
}

size_t DownloadedCacheLoaderCallback::RxBodyData(void* buffer, size_t size, size_t nitems, void* userParam)
{
    FALSE_RETURN_V_MSG_E(buffer != nullptr, 0, "buffer is nullptr");
    FALSE_RETURN_V_MSG_E(userParam != nullptr, 0, "userParam is nullptr");
    auto mediaDownloader = static_cast<DownloadedCacheLoaderCallback*>(userParam);
    ...
    if (mediaDownloader->isFirstCallback_) {
        ...
        mediaDownloader->task_->SubmitJobOnce([mediaDownloader] {
            FALSE_RETURN_MSG(mediaDownloader->DownloadedRequestHandler_->GetClient() != nullptr, "client is nullptr!");
            mediaDownloader->DownloadedRequestHandler_->GetClient()->Close(false);
            mediaDownloader->DownloadedRequestHandler_->GetClient()->Deinit();
            mediaDownloader->DownloadedRequestHandler_->SetClient(nullptr);
    ...
    mediaDownloader->DownloadedRequestHandler_->OnBodyReceived(buffer, size, nitems);
    return dataLen;
}

void DownloadedCacheLoaderCallback::AppendToBuffer(DownloadedCacheLoaderCallback* downloader, const uint8_t* buffer, int64_t dataLen)
{
...
void DownloadedCacheLoaderCallback::RemoveCacheFile()
{
    MEDIA_LOG_I("RemoveCacheFile in");
    isSupportLocalCache_ = false;
}

void DownloadedCacheLoaderCallback::DownloadUnsupportRangeAndCache(int64_t uuid,
    int64_t requestedLength, const std::weak_ptr<DownloadedCacheLoaderCallback>& weakThis)
{
    // 1. 初始化客户端（仅第一次）
    if (!DownloadedRequestHandler_->GetClient() || firstRequest_) {
        InitializeClient();
    }
...
void DownloadedCacheLoaderCallback::InitializeClient()
{
    MEDIA_LOG_I("first request set RxBodyDataUnsupportRangeAndCache");
    auto client = DownloadedNetworkClientAgent::NewInstance(&RxHeaderData, &RxBodyDataUnsupportRangeAndCache, this);
        FALSE_RETURN_MSG(client != nullptr, "client is null");
    client->Init();
    DownloadedRequestHandler_->SetClient(client);
    firstRequest_ = false;
...
void DownloadedCacheLoaderCallback::StartDownloadTask(const std::weak_ptr<DownloadedCacheLoaderCallback>& weakThis)
{
...
void DownloadedCacheLoaderCallback::ReadDataTask(const std::weak_ptr<DownloadedCacheLoaderCallback>& weakThis,
...
void DownloadedCacheLoaderCallback::HandleInitialization(int64_t& len, bool& noSizeAndRequest,
    const std::shared_ptr<DownloadedCacheLoaderCallback>& self)
{
    if (len == -1) {
        len = self->size_;
    }
...
void DownloadedCacheLoaderCallback::HandleCacheMiss(int64_t start, int64_t& len,
    bool noSizeAndRequest, const std::shared_ptr<DownloadedCacheLoaderCallback>& self)
{
...
void DownloadedCacheLoaderCallback::HandleDownloadFromStart(int64_t& len, bool noSizeAndRequest,
    const std::shared_ptr<DownloadedCacheLoaderCallback>& self, const std::weak_ptr<DownloadedCacheLoaderCallback>& weakThis)
{
...
void DownloadedCacheLoaderCallback::RespondDataChunk(int64_t start, int64_t& read, int64_t& len,
    bool noSizeAndRequest, const std::shared_ptr<DownloadedCacheLoaderCallback>& self)
{
...
size_t DownloadedCacheLoaderCallback::RxBodyDataUnsupportRangeAndCache(void* buffer, size_t size, size_t nitems, void* userParam)
{
    FALSE_RETURN_V_MSG_E(buffer != nullptr, 0, "buffer is nullptr");
    FALSE_RETURN_V_MSG_E(userParam != nullptr, 0, "userParam is nullptr");
    auto downloader = static_cast<DownloadedCacheLoaderCallback*>(userParam);
    ...
    if (dataLen + static_cast<size_t>(downloader->dataSize) > SHARD_SIZE) {
        ...
        errno_t rc = memcpy_s(downloader->buffer_ + downloader->dataSize, SHARD_SIZE -
            downloader->dataSize, tempBuffer, dataLen);
...
void DownloadedCacheLoaderCallback::Redownload(const std::weak_ptr<DownloadedCacheLoaderCallback>& weakThis)
{
    interruptedTask_->SubmitJobOnce([weakThis] {
        ...
        FALSE_RETURN_MSG(self->DownloadedRequestHandler_->GetClient() != nullptr, "client is nullptr");
        self->DownloadedRequestHandler_->GetClient()->Close(false);
        self->DownloadedRequestHandler_->GetClient()->Deinit();
        self->DownloadedRequestHandler_->SetClient(nullptr);
        self->isInterruptedNewDownLoad_ = true;
    });
    task_->SubmitJobOnce([weakThis] {
        auto self = weakThis.lock();
        ...
        self->cacheOffset_ = 0;
        memset_s(self->buffer_, SHARD_SIZE, 0, SHARD_SIZE);
        auto client = DownloadedNetworkClientAgent::NewInstance(&RxHeaderData, &RxBodyData, self.get());
        FALSE_RETURN_MSG(client != nullptr, "client is null");
        client->Init();
        self->DownloadedRequestHandler_->SetClient(client);
