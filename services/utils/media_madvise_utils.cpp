...
size_t MediaMadviseUtils::PageSize()
{
    static const size_t pageSize = static_cast<size_t>(::getpagesize());
    return pageSize;
}

bool MediaMadviseUtils::ShouldOptimizeSegment(uint32_t flags)
{
...
bool MediaMadviseUtils::ApplyMadviseAligned(void *addr, size_t len)
{
...
int32_t MediaMadviseUtils::PhdrCallbackSingle(struct dl_phdr_info *info, size_t, void *data)
{
...
bool MediaMadviseUtils::MediaMadviseSingleLibrary(const std::string &libName)
{
    if (libName.empty()) {
    ...
    MEDIA_LOG_I("MadviseSingleLibrary done: success=%{public}d fail=%{public}d", ctx.successCount, ctx.failCount);
    return ctx.successCount > 0;
}

int32_t MediaMadviseUtils::PhdrCallbackMultiple(struct dl_phdr_info *info, size_t, void *data)
{
...
int32_t MediaMadviseUtils::MediaMadviseMultipleLibraries(const std::vector<std::string> &libNames)
{
    if (libNames.empty()) {
    ...
    MEDIA_LOG_I("MadviseMultipleLibraries done: success=%{public}d fail=%{public}d", ctx.successCount, ctx.failCount);
