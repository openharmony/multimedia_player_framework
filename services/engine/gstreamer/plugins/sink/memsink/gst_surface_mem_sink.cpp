    ...
    for (guint i = 0; i < gst_buffer_n_memory(buffer); i++) {
        ...
        if (needFlush) {
            ...
            gst_surface_mem_sink_dump_buffer(surface_sink, buffer);
            {
                ...
                LISTENER(ret = priv->surface->FlushBuffer(surface_mem->buf,
                    surface_mem->fence, flushConfig), "surface::FlushBuffer", PlayerXCollie::timerTimeout)
    ...
    if (surface_sink->firstRenderFrame) {
        ...
        if (surface_sink->priv->surface) {
            ...
            LISTENER((void)surface_sink->priv->surface->SetTransform(
                ...
                "surface::SetTransform", PlayerXCollie::timerTimeout)
        }
    }
