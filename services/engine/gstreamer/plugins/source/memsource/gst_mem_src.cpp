...
static gboolean gst_mem_src_query(GstBaseSrc *src, GstQuery *query);
...
static void gst_mem_src_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
    ...
    switch (prop_id) {
        case PROP_CAPS:
            gst_mem_src_set_caps(memsrc, gst_value_get_caps(value));
            break;
        case PROP_EMIT_SIGNALS:
            gst_mem_src_set_emit_signals(memsrc, g_value_get_boolean(value));
            break;
        case PROP_BUFFER_SIZE:
            gst_mem_src_set_buffer_size(memsrc, g_value_get_uint(value));
            break;
