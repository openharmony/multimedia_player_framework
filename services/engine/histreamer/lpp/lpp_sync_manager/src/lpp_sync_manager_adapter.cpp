...
int32_t LppSyncManagerAdapter::LoadAdapter()
{
    ...
    FALSE_RETURN_V_MSG_E(factory_ != nullptr, MSERR_NO_MEMORY, "syncMgrfactory is nullptr");
    int32_t ret = factory_->CreateSyncMgr(syncMgrAdapter_);
    FALSE_RETURN_V_MSG(ret == HDF_SUCCESS && syncMgrAdapter_ != nullptr, MSERR_NO_MEMORY,
        "Create SyncMgrAdapter failed");
    return MSERR_OK;
}

int32_t LppSyncManagerAdapter::UnloadAdapter()
{
    MediaTrace trace("LppSyncManagerAdapter::UnloadAdapter");
    MEDIA_LOG_I("UnloadAdapter enter");
    FALSE_RETURN_V_MSG_E(factory_ != nullptr, MSERR_INVALID_OPERATION, "syncMgrfactory is nullptr");
    factory_ = nullptr;
...
int32_t LppSyncManagerAdapter::Init()
{
    ...
    FALSE_RETURN_V_MSG(ret == HDF_SUCCESS, MSERR_HARDWARE_ERROR, "SyncMananger RegisterCallback failed");
    return MSERR_OK;
}
...
int32_t LppSyncManagerAdapter::Pause()
{
    ...
    FALSE_RETURN_V_MSG(ret == HDF_SUCCESS, MSERR_HARDWARE_ERROR, "SyncMananger Pause failed, ret");
    return MSERR_OK;
}
...
int32_t LppSyncManagerAdapter::Resume()
{
    ...
    FALSE_RETURN_V_MSG(ret == HDF_SUCCESS, MSERR_HARDWARE_ERROR, "SyncMananger Resume failed");
    return MSERR_OK;
}
...
int32_t LppSyncManagerAdapter::Reset()
{
    ...
    FALSE_RETURN_V_MSG(ret == HDF_SUCCESS, MSERR_HARDWARE_ERROR, "SyncMananger Reset failed, ret");
    return MSERR_OK;
}
...
int32_t LppSyncManagerAdapter::GetParameter(std::map<std::string, std::string> &parameters)
{
    ...
    FALSE_RETURN_V_MSG(ret == HDF_SUCCESS && !parameters.empty(), MSERR_HARDWARE_ERROR,
...
int32_t LppSyncManagerAdapter::UpdateTimeAnchor(const int64_t anchorPts, const int64_t anchorClk)
{
    ...
    FALSE_RETURN_V_MSG_E(syncMgrAdapter_ != nullptr, MSERR_INVALID_OPERATION, "syncMgrAdapter_ is nullptr");
    FALSE_RETURN_V_MSG_E(anchorPts >= 0 && anchorClk >= 0, MSERR_INVALID_VAL, "anchor is invalid");
    int32_t ret = syncMgrAdapter_->UpdateTimeAnchor(static_cast<uint64_t>(anchorPts), static_cast<uint64_t>(anchorClk));
    FALSE_RETURN_V_MSG(ret == HDF_SUCCESS, MSERR_HARDWARE_ERROR, "SyncMananger UpdateTimeAnchor failed, ret");
    return MSERR_OK;
}
...
int32_t LppSyncManagerAdapter::BindOutputBuffers(const std::map<uint32_t, sptr<SurfaceBuffer>> &bufferMap)
{
    ...
    FALSE_RETURN_V_MSG_E(syncMgrAdapter_ != nullptr, MSERR_INVALID_OPERATION, "syncMgrAdapter_ is nullptr");
    FALSE_RETURN_V_MSG_E(!bufferMap.empty(), MSERR_INVALID_VAL, "bufferMap is empty");
    ...
    FALSE_RETURN_V_MSG(ret == HDF_SUCCESS, MSERR_HARDWARE_ERROR, "SyncMananger BindOutputBuffers failed with error");
    return MSERR_OK;
}
...
int32_t LppSyncManagerAdapter::GetShareBuffer(int32_t &fd)
{
    ...
    FALSE_RETURN_V_MSG(ret == HDF_SUCCESS, MSERR_HARDWARE_ERROR, "SyncMananger GetShareBuffer failed with error");
    return MSERR_OK;
}
...
int32_t LowPowerPlayerFactory::CreateLppSyncManagerAdapter(std::shared_ptr<LppSyncManagerAdapter> &adapter)
{
    ...
    int32_t ret = syncAadapter->LoadAdapter();
    FALSE_RETURN_V_MSG(ret == MSERR_OK, MSERR_HARDWARE_ERROR, "LoadAdapter failed");
    adapter = std::move(syncAadapter);
    return MSERR_OK;
}

int32_t LowPowerPlayerFactory::DestroyLppSyncManagerAdapter(std::shared_ptr<LppSyncManagerAdapter> adapter)
{
    ...
    int32_t ret = adapter->UnloadAdapter();
    FALSE_RETURN_V_MSG_E(ret == MSERR_OK, MSERR_HARDWARE_ERROR, "Failed to Destroy LoadAdapter");
    return MSERR_OK;
}
