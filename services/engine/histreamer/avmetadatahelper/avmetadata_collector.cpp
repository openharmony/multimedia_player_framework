    ...
    const std::shared_ptr<Meta> &globalInfo, const std::vector<std::shared_ptr<Meta>> &trackInfos)
{
    ...
    for (size_t index = 0; index < trackCount; index++) {
        ...
        CHECK_AND_RETURN_RET_LOG(meta->GetData(Tag::MEDIA_TYPE, mediaType), metadata.tbl_, "mediaType not found, index: %zu", index);
...
std::shared_ptr<AVSharedMemory> AVMetaDataCollector::GetArtPicture()
{
    ...
    for (size_t index = 0; index < trackCount; index++) {
        ...
        CHECK_AND_RETURN_RET_LOG(!(coverAddr.size() == 0 || static_cast<int>(coverAddr.size()) > PICTURE_MAX_SIZE), nullptr, "InvalidArtPictureSize %d", coverAddr.size());
